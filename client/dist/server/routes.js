...
import { analyzeImageForContent } from "./services/ai-generator.js";
...
import { getRandomTemplates, addWatermark } from "./content-templates.js";
import { generateAdvancedContent } from "./advanced-content-generator.js";
...
export async function registerRoutes(app) {
    ...
    app.post("/api/auth/signup", async (req, res) => {
        try {
            ...
            // Generate JWT token
            const token = jwt.sign({ id: newUser.id, userId: newUser.id, username: newUser.username, email: newUser.email }, JWT_SECRET, { expiresIn: '24h' });
    ...
    app.post("/api/auth/login", async (req, res) => {
        try {
            ...
            if (email === 'admin@thottopilot.com' && password === 'admin123') {
                ...
                const token = jwt.sign({ userId: adminUser.id, id: adminUser.id, email: adminUser.email, username: adminUser.username, isAdmin: true }, JWT_SECRET, { expiresIn: '24h' });
                return res.json({
            ...
            // Generate JWT token
            const token = jwt.sign({ id: user.id, userId: user.id, username: user.username, email: user.email }, JWT_SECRET, { expiresIn: '24h' });
            // Remove password from response
    ...
    app.get("/api/pro-perks", async (req, res) => {
        try {
            ...
            let perks = getAvailablePerks(userTier);
            if (category && category !== 'all') {
                perks = getPerksByCategory(category);
            }
            res.json({
                perks,
    ...
    app.post("/api/pro-perks/:perkId/generate-referral", authenticateToken, async (req, res) => {
        try {
            const { perkId } = req.params;
    ...
    app.post('/api/generate-unified', upload.single('image'), async (req, res) => {
        try {
    ...
    app.post("/api/generate", async (req, res) => {
        try {
            ...
            const generatedContent = await generateContent(validatedData.platform, validatedData.style, validatedData.theme, req.body.timing, req.body.allowsPromotion);
    ...
    app.get("/api/reddit-communities", async (req, res) => {
        try {
            ...
            if (search) {
                ...
                communities = communities.filter(c => c.name.toLowerCase().includes(searchTerm) ||
            ...
            if (userStyle && experience) {
                communities = getRecommendationsForUser(userStyle, experience);
            }
    ...
    app.post("/api/generate-ai", upload.single('image'), async (req, res) => {
        try {
            const { generationType, platform, customPrompt, subreddit, allowsPromotion, userProfile, style, theme, preferredProvider, photoType, textTone, includePromotion, selectedHashtags } = req.body;
            ...
            if (userTier === 'free' || userTier === 'basic') {
                ...
                contentGeneration.contentSource = 'template';
                contentGeneration.userTier = userTier;
                contentGeneration.upgradeMessage = userTier === 'free'
                ...
                if (req.file && generationType === 'ai-image') {
                    ...
                    imageDescription = await analyzeImageForContent(fullImageUrl, { personalityProfile: parsedProfile });
                }
                // Generate AI content using multi-provider system
                const aiContent = await generateWithMultiProvider({
                    user: { personalityProfile: parsedProfile, preferences: parsedProfile },
                    platform,
                    ...
                    for (let i = 1; i < variationCount; i++) {
                        try {
                            const variation = await generateWithMultiProvider({
                                user: { personalityProfile: { ...parsedProfile, variation: i } },
                                platform,
                                imageDescription: imageDescription || undefined,
                ...
                contentGeneration.aiProvider = allVariations.map(v => v.provider).join(', ');
                contentGeneration.estimatedCost = allVariations.reduce((sum, v) => sum + (v.estimatedCost || 0), 0);
                contentGeneration.contentSource = 'ai';
                contentGeneration.userTier = userTier;
                contentGeneration.variationCount = allVariations.length;
            }
            res.json(contentGeneration);
        }
        catch (error) {
