...
export function setupAuth(app) {
    ...
    app.post('/api/auth/signup', async (req, res) => {
        try {
            ...
            // Generate token
            const token = jwt.sign({ id: user.id, userId: user.id, username: user.username }, JWT_SECRET, { expiresIn: '24h' });
            // Send welcome email
            if (user.email) {
    ...
    app.post('/api/auth/login', async (req, res) => {
        try {
            ...
            if (loginEmail === 'admin@thottopilot.com' && password === 'admin123') {
                ...
                const token = jwt.sign({ id: adminUser.id, userId: adminUser.id, username: adminUser.username, email: adminUser.email }, JWT_SECRET, { expiresIn: '24h' });
                return res.json({
