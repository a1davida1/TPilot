 ...
 */
import { generateWithMultiProvider } from './multi-ai-provider.js';
import { preGeneratedTemplates } from '../content-templates.js';
class EnhancedAIContentGenerator {
    ...
    buildEnhancedPrompt(request) {
        const { mode, prompt, platform, style, theme, tone, contentType, includePromotion, promotionLevel, targetAudience, customInstructions, subreddit, niche, personalBrand } = request;
...
`;
    ...
    async tryAIGeneration(prompt, request) {
        try {
    ...
    enhanceResponse(baseResponse, request, metadata) {
        ...
        const photoInstructions = this.createDetailedPhotoInstructions(baseResponse.photoInstructions || {}, request);
        return {
    ...
    createDetailedPhotoInstructions(base, request) {
    ...
    generateOptimizedHashtags(request, base = []) {
    ...
    generateTitles(request) {
        const templates = preGeneratedTemplates.filter(t => t.style === request.style &&
    ...
    generateEngagementHooks(request) {
        return [
    ...
    suggestOptimalPostingTime(platform) {
        const times = {
    ...
    suggestCrossPromotion(currentPlatform) {
        const suggestions = {
    ...
    estimateEngagement(request, content) {
        let score = 0;
        // Check for engagement factors
        if (content.includes('?'))
            score++; // Questions increase engagement
        if (request.includePromotion && request.promotionLevel === 'subtle')
            score++; // Subtle promotion works better
        if (request.style === 'playful' || request.style === 'authentic')
            score++; // These styles typically perform better
        if (content.length > 50 && content.length < 200)
            score++; // Optimal length
        if (score >= 3)
            return 'high';
        if (score >= 2)
            return 'medium';
    ...
    generateSafetyFallback(request) {
        return {
            titles: [
...
export async function generateEnhancedContent(request) {
