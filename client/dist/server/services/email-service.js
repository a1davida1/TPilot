...
const emailTemplates = {
    ...
    upgradeConfirmation: (username, plan) => ({
    ...
    contentGenerated: (username, platform) => ({
...
export class EmailService {
    ...
    async sendPasswordResetEmail(email, username) {
        ...
        try {
            ...
            const resetToken = jwt.sign({ email, username, type: 'password-reset' }, JWT_SECRET, { expiresIn: '1h' });
    ...
    async sendUpgradeEmail(email, username, plan) {
        if (!this.isConfigured) {
    ...
    async sendContentNotification(email, username, platform) {
        if (!this.isConfigured) {
    ...
    async sendVerificationEmail(email, username) {
        ...
        try {
            const verificationToken = jwt.sign({ email, username, type: 'email-verification' }, JWT_SECRET, { expiresIn: '24h' });
            await sgMail.send({
                to: email,
                from: 'noreply@thottopilot.com',
