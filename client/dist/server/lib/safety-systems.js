...
import { postRateLimits, postDuplicates } from '../../shared/schema.js';
...
export class SafetyManager {
     ...
     */
    static async checkRateLimit(userId, subreddit) {
        const now = new Date();
        ...
        const [currentLimit] = await db
            .select()
            .from(postRateLimits)
            .where(and(eq(postRateLimits.userId, parseInt(userId)), eq(postRateLimits.subreddit, subreddit), gte(postRateLimits.lastPostAt, windowStart)))
        ...
        if (postsInWindow >= maxPosts) {
            const nextAvailableTime = new Date((currentLimit?.lastPostAt?.getTime() || now.getTime()) + 24 * 60 * 60 * 1000);
            return {
                canPost: false,
     ...
     */
    static async recordPost(userId, subreddit) {
        const now = new Date();
     ...
     */
    static async checkDuplicate(userId, subreddit, title, body) {
        ...
        const [duplicate] = await db
            .select()
            .from(postDuplicates)
            .where(and(eq(postDuplicates.userId, parseInt(userId)), eq(postDuplicates.contentHash, contentHash), gte(postDuplicates.createdAt, thirtyDaysAgo)))
     ...
     */
    static async recordPostForDuplicateDetection(userId, subreddit, title, body) {
        const contentHash = this.generateContentHash(title, body);
     ...
     */
    static async getUserPostingStats(userId) {
        const now = new Date();
        ...
        const [postsLast24h] = await db
            ...
            .from(postDuplicates)
            .where(and(eq(postDuplicates.userId, parseInt(userId)), gte(postDuplicates.createdAt, oneDayAgo)));
        ...
        const [postsLast7d] = await db
            ...
            .from(postDuplicates)
            .where(and(eq(postDuplicates.userId, parseInt(userId)), gte(postDuplicates.createdAt, sevenDaysAgo)));
        // Get top subreddits
     ...
     */
    static generateContentHash(title, body) {
     ...
     */
    static async getUserSafetyStatus(userId) {
