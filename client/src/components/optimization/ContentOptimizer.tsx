    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="insights" className="space-y-4">
        ...
        <TabsContent value="insights" className="space-y-4">
          ...
          <div className="grid gap-4">
            {optimizationInsights.map((insight) => (
              <Card key={insight.id} className="bg-gradient-to-r from-gray-900/50 to-gray-800/50 border-gray-700/50">
        ...
        <TabsContent value="testing" className="space-y-4">
          <div className="grid gap-4">
            {abTests.map((test) => (
              <Card key={test.id} className="bg-gradient-to-r from-gray-900/50 to-gray-800/50 border-gray-700/50">
                <CardHeader className="flex flex-row items-center justify-between space-y-0">
                  <div>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      {test.improvement && (
                        <span className="text-sm text-green-400">{test.improvement}</span>
                      )}
                      {test.status === 'running' && (
                        <span className="text-sm text-gray-400">Test in progress...</span>
                      )}
                      {test.status === 'pending' && (
                        <span className="text-sm text-gray-400">Waiting to start</span>
                      )}
                    </div>
                    <div className="flex space-x-2">
                      {test.status === 'completed' && (
                        <Button variant="outline" size="sm" data-testid={`view-results-${test.id}`}>
                          View Results
                        </Button>
                      )}
                      {test.status === 'running' && (
                        <Button variant="outline" size="sm" data-testid={`stop-test-${test.id}`}>
                          Stop Test
                        </Button>
                      )}
                      {test.status === 'pending' && (
                        <Button size="sm" className="bg-purple-600 hover:bg-purple-700" data-testid={`start-test-${test.id}`}>
                          Start Test
                        </Button>
                      )}
                    </div>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <Slider
                    ...
                    step={5}
                    className="w-full"
                    data-testid="slider-optimization-level"
                  />
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    data-testid="switch-real-time-optimization"
                  />
                </div>
              </CardContent>
            </Card>
