...
export async function registerRoutes(app) {
    ...
    app.post("/api/generate-content", generationLimiter, authenticateToken, async (req, res) => {
        try {
            if (!req.user?.id) {
                return res.status(401).json({ message: "Authentication required" });
            }
            const { platform, style, theme, timing, allowsPromotion } = req.body;
            const result = await generateContent(platform || 'reddit', style || 'playful', theme || 'lingerie', timing, allowsPromotion);
            // Save to database
            ...
            const photoInstructions = {
                ...
                cameraAngle: Array.isArray(result.photoInstructions.angles)
                    ...
                    : result.photoInstructions.cameraAngle || 'Eye level',
                ...
                technicalSettings: Array.isArray(result.photoInstructions.technical)
                    ...
                    : result.photoInstructions.technicalSettings || 'Auto settings'
            };
    ...
    app.post('/api/generate-unified', generationLimiter, authenticateToken, upload.single('image'), async (req, res) => {
        try {
    ...
    app.get("/api/user-stats", authenticateToken, async (req, res) => {
        try {
            ...
            const todayGenerations = generations.filter(g => g.createdAt && new Date(g.createdAt).toDateString() === today.toDateString());
            const stats = {
                total: generations.length,
                today: todayGenerations.length,
                thisWeek: generations.filter(g => g.createdAt &&
                    new Date(g.createdAt) > new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000)).length,
                thisMonth: generations.filter(g => g.createdAt &&
                    new Date(g.createdAt).getMonth() === today.getMonth() &&
                    new Date(g.createdAt).getFullYear() === today.getFullYear()).length,
    ...
    app.get("/api/content-generation-history", authenticateToken, async (req, res) => {
        try {
            if (!req.user?.id) {
                return res.status(401).json({ message: "Authentication required" });
            }
    ...
    app.get('/api/content-generations', authenticateToken, async (req, res) => {
        try {
            if (!req.user?.id) {
                return res.status(401).json({ message: "Authentication required" });
            }
    ...
    app.delete('/api/content-generations/:id', authenticateToken, async (req, res) => {
        try {
            if (!req.user?.id) {
                return res.status(401).json({ message: "Authentication required" });
            }
    ...
    app.post('/api/billing/payment-link', authenticateToken, async (req, res) => {
        try {
            const { plan } = req.body;
    ...
    app.get('/api/reddit/communities', authenticateToken, async (req, res) => {
        try {
            const communities = [
    ...
    app.post('/api/social-media/quick-post', authenticateToken, async (req, res) => {
        try {
    ...
    app.post('/api/protect-image/:imageId', authenticateToken, async (req, res) => {
        try {
