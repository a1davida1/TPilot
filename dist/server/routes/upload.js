...
async function applyImageShieldProtection(inputPath, outputPath, protectionLevel = 'standard', addWatermark = false) {
...
function performBasicMalwareCheck(buffer) {
...
router.post('/image', uploadLimiter, tierProtectionLimiter, authenticateToken, upload.single('image'), async (req, res) => {
    ...
    try {
        ...
        await applyImageShieldProtection(tempFilePath, protectedFilePath, protectionLevel, addWatermark);
        ...
        if (protectedFilePath) {
            try {
                await fs.unlink(protectedFilePath);
            }
            catch (e) { /* ignore cleanup errors */ }
        }
