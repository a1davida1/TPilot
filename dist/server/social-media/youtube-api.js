...
export class YouTubeAPI {
    ...
    async getVideoMetrics(videoId) {
        try {
            const response = await fetch(`https://www.googleapis.com/youtube/v3/videos?id=${videoId}&part=statistics,snippet&key=${this.accessToken}`, {
                headers: {
                    'Authorization': `Bearer ${this.accessToken}`,
                },
            });
    ...
    async getChannelMetrics(channelId) {
        try {
            ...
            const response = await fetch(`https://www.googleapis.com/youtube/v3/channels?${channelQuery}&part=statistics,snippet`, {
                headers: {
                    'Authorization': `Bearer ${this.accessToken}`,
                },
            });
    ...
    async searchVideos(query, maxResults = 25) {
        try {
            const response = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&q=${encodeURIComponent(query)}&type=video&maxResults=${maxResults}`, {
                headers: {
                    'Authorization': `Bearer ${this.accessToken}`,
                },
            });
