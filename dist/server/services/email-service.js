...
export const emailService = {
    ...
    async sendPasswordResetEmail(to, username) {
        if (!SENDGRID_API_KEY) {
        ...
        const resetToken = jwt.default.sign({ email: to, type: 'password-reset' }, JWT_SECRET, { expiresIn: '1h' });
    ...
    async sendUpgradeEmail(to, username, newTier) {
        if (!SENDGRID_API_KEY) {
            // SendGrid not configured - email skipped
            return;
        }
    ...
    async sendAdminWaitlistNotification(email, platformTags, painPoint, utmData) {
        if (!SENDGRID_API_KEY) {
