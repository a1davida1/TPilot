...
class AnalyticsService {
    ...
    async trackSignup(userId, source) {
    ...
    async trackLogin(userId, method) {
    ...
    async trackContentGenerated(userId, platform, style) {
    ...
    async trackPostCreated(userId, platform, success) {
    ...
    async trackSubscription(userId, plan, revenue) {
    ...
    async trackPageView(userId, page, referrer) {
    ...
    async getDailyActiveUsers() {
        ...
        try {
            const result = await db.select({ count: sql `count(*)` })
                .from(users)
                .where(and(gte(users.lastLogin, today), lte(users.lastLogin, new Date())));
    ...
    async getMonthlyActiveUsers() {
        ...
        try {
            const result = await db.select({ count: sql `count(*)` })
                .from(users)
                .where(and(gte(users.lastLogin, thirtyDaysAgo), lte(users.lastLogin, new Date())));
            return result[0]?.count || 0;
        }
        catch (error) {
    ...
    async getFeatureAdoption() {
        ...
        const features = [
            { feature: 'AI Content Generation', users: 1250, percentage: 75, trend: 'up' },
            { feature: 'Image Protection', users: 890, percentage: 53, trend: 'up' },
            { feature: 'Post Scheduling', users: 650, percentage: 39, trend: 'stable' },
            { feature: 'Analytics Dashboard', users: 450, percentage: 27, trend: 'up' },
            { feature: 'Bulk Operations', users: 230, percentage: 14, trend: 'down' }
        ];
        return features;
    }
    ...
    // A/B test tracking
    async trackABTestExposure(userId, testName, variant) {
    ...
    async trackABTestConversion(userId, testName, variant, value) {
