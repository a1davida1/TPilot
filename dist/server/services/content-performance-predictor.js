class ContentPerformancePredictor {
    ...
    applyPlatformAdjustments(score, features) {
        const multipliers = this.platformMultipliers[features.platform];
        if (!multipliers)
            return score;
        let adjustment = 1.0;
    ...
    applyContentTypeAdjustments(score, features) {
    ...
    calculateConfidence(features) {
        ...
        if (features.titleLength > 0)
            completeness++;
        totalFields++;
        if (features.contentLength > 0)
            completeness++;
        totalFields++;
        if (features.imageCount !== undefined)
            completeness++;
        totalFields++;
        if (features.hashtagCount !== undefined)
            completeness++;
        totalFields++;
    ...
    async updateModel(actualPerformance) {
