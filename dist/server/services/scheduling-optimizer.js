...
class SchedulingOptimizer {
    ...
    async getBestPostTime(userId, platform) {
        try {
    ...
    async getUserPostHistory(userId, platform) {
        try {
            const posts = await db.select({
                ...
                .where(and(eq(socialMediaPosts.userId, userId), eq(socialMediaPosts.platform, platform), eq(socialMediaPosts.status, 'published')))
    ...
    getDefaultBestTime(platform) {
        const times = this.defaultBestTimes[platform]
    ...
    async suggestContentBasedOnTrends(userId) {
        try {
    ...
    async optimizePostingSchedule(userId, posts) {
        ...
        return optimizedPosts.sort((a, b) => a.scheduledTime.getTime() - b.scheduledTime.getTime());
    }
    calculateOptimizationScore(time, platform) {
        ...
        const bestDays = this.bestDays[platform] || [1, 2, 3, 4, 5];
        ...
        for (const platform of platforms) {
            ...
            if (history.length === 0) {
                ...
                stats.push({
                    platform,
                    avgEngagement: 0,
                    bestHours: this.defaultBestTimes[platform]?.weekdays.slice(0, 3) || [],
                    bestDays: this.bestDays[platform] || [],
                    topHashtags: []
                });
                continue;
            }
