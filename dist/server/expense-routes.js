...
async function applyReceiptImageShieldProtection(inputBuffer, protectionLevel = 'light', addWatermark = false) {
    try {
...
export function registerExpenseRoutes(app) {
    ...
    app.get('/api/expenses', authenticateToken, async (req, res) => {
        try {
            ...
            const taxYear = req.query.taxYear ? parseInt(req.query.taxYear) : undefined;
    ...
    app.get('/api/expenses/totals', authenticateToken, async (req, res) => {
        try {
            if (!req.user?.id) {
                return res.status(401).json({ message: 'Authentication required' });
            }
    ...
    app.post('/api/expenses/:id/receipt', authenticateToken, upload.single('receipt'), async (req, res) => {
        try {
            ...
            const protectedBuffer = await applyReceiptImageShieldProtection(req.file.buffer, protectionLevel, addWatermark);
            let receiptUrl;
