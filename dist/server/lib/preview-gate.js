...
export async function getPreviewStats(userId) {
    try {
        ...
        const okPreviews = await db
            .select()
            .from(postPreviews)
            .where(and(eq(postPreviews.userId, userId), eq(postPreviews.policyState, "ok"), gte(postPreviews.createdAt, fourteenDaysAgo)));
        // Count total previews in last 14 days (for additional context)
        const totalPreviews = await db
            .select()
            .from(postPreviews)
            .where(and(eq(postPreviews.userId, userId), gte(postPreviews.createdAt, fourteenDaysAgo)));
...
export async function canQueuePosts(userId) {
    const stats = await getPreviewStats(userId);
    return stats.canQueue;
}
export async function checkPreviewGate(userId) {
    const stats = await getPreviewStats(userId);
    if (stats.canQueue) {
