...
export class MetricsWorker {
    ...
    async initialize() {
        if (this.initialized)
            return;
        await registerProcessor(QUEUE_NAMES.METRICS, this.processJob.bind(this), { concurrency: 3 } // Process 3 metrics jobs at once
        );
        this.initialized = true;
        logger.info('✅ Metrics worker initialized with queue abstraction');
    }
    async processJob(jobData, jobId) {
        const { postJobId, redditPostId, scheduledFor } = jobData;
        try {
    ...
    async fetchPostMetrics(reddit, redditPostId) {
        try {
    ...
    async updatePostMetrics(postJobId, metrics) {
        try {
    ...
    async scheduleNextCheck(postJobId, redditPostId, daysSincePost) {
        try {
