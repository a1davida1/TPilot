...
export class DunningWorker {
    ...
    async initialize() {
        if (this.initialized)
            return;
        await registerProcessor(QUEUE_NAMES.DUNNING, this.processJob.bind(this), { concurrency: 1 } // Process 1 dunning job at a time to avoid overwhelming payment systems
        );
        this.initialized = true;
        logger.info('✅ Dunning worker initialized with queue abstraction');
    }
    async processJob(jobData, jobId) {
        const { subscriptionId, attempt, maxAttempts = 3 } = jobData;
        try {
    ...
    async retryStripePayment(subscription) {
        try {
    ...
    async retryCCBillPayment(subscription) {
        try {
    ...
    async scheduleNextAttempt(subscriptionId, nextAttempt, maxAttempts) {
        try {
    ...
    async sendRecoveryNotification(user, subscription) {
    ...
    async sendSuspensionNotification(user, subscription) {
        // In full implementation, this would send an email
