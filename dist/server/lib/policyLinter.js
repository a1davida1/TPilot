...
export class PolicyLinter {
    rules;
    subreddit;
    constructor(subreddit, rules) {
        this.subreddit = subreddit.toLowerCase();
        this.rules = { ...DEFAULT_RULES, ...rules };
    }
    static async forSubreddit(subreddit) {
        try {
            const rule = await db
                .select()
                .from(subredditRules)
    ...
    async lintPost(title, body, hasImage = false) {
        const warnings = [];
        let score = 100;
        // Check title length
        ...
        const bannedWordsFound = (this.rules.bannedWords || []).filter(word => fullText.includes(word.toLowerCase()));
...
export async function updateSubredditRules(subreddit, rules) {
    await db
