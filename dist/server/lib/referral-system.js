...
export class ReferralManager {
    ...
    static async getReferralInfo(userId) {
        ...
        const [stats] = await db
            .select({
            totalReferrals: sql `COUNT(*)`.as('totalReferrals'),
            activeReferrals: sql `COUNT(*) FILTER (WHERE subscription_status IS NOT NULL AND subscription_status != 'inactive')`.as('activeReferrals'),
        })
            .from(users)
     ...
     */
    static async processReferralReward(subscribingUserId) {
     ...
     */
    static async generateReferralLink(userId, baseUrl) {
        const code = await this.getUserReferralCode(userId);
    ...
    static async getReferralLeaderboard(limit = 10) {
        const results = await db
            ...
            activeReferrals: sql `COUNT(referrals.id) FILTER (WHERE referrals.subscription_status IS NOT NULL AND referrals.subscription_status != 'inactive')`.as('activeReferrals'),
        })
            .from(users)
