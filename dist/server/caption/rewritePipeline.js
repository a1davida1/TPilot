...
import { textModel, visionModel } from "../lib/gemini.js";
...
export async function variantsRewrite(params) {
    ...
    if (Array.isArray(json)) {
        json.forEach((item) => {
            ...
            if (!item.alt || item.alt.length < 20)
                item.alt = "Engaging social media content";
            if (!item.hashtags || !Array.isArray(item.hashtags))
                item.hashtags = ["#content", "#creative", "#amazing"];
            if (!item.caption || item.caption.length < 1)
                item.caption = "Check out this amazing content, you'll love it and want more!";
        });
...
export async function rankAndSelect(variants) {
    ...
    if (json.final) {
        if (!json.final.safety_level)
            json.final.safety_level = "suggestive";
        ...
        if (!json.final.alt || json.final.alt.length < 20)
            json.final.alt = "Engaging social media content";
        if (!json.final.hashtags || !Array.isArray(json.final.hashtags))
            json.final.hashtags = ["#content", "#creative", "#amazing"];
        if (!json.final.caption || json.final.caption.length < 1)
            json.final.caption = "Check out this amazing content!";
    }
