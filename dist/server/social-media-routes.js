...
export function registerSocialMediaRoutes(app) {
    ...
    app.delete("/api/social-media/accounts/:accountId", authenticateToken, async (req, res) => {
        try {
            if (!req.user?.id) {
                return res.status(401).json({ message: "Authentication required" });
            }
            ...
            socialMediaManager.disconnect(account.platform);
    ...
    app.post("/api/social-media/post", authenticateToken, async (req, res) => {
        try {
            if (!req.user?.id) {
                return res.status(401).json({ message: "Authentication required" });
            }
            ...
            for (const account of userAccounts) {
                if (connectedPlatforms.includes(account.platform) && account.accessToken) {
                    ...
                    socialMediaManager.connectAccount(account.platform, credentials);
                }
            }
            let results;
            if (scheduledAt) {
                ...
                results = await socialMediaManager.postToMultiplePlatforms(connectedPlatforms, postContent);
    ...
    app.get("/api/social-media/metrics/:postId", authenticateToken, async (req, res) => {
        try {
            if (!req.user?.id) {
                return res.status(401).json({ message: "Authentication required" });
            }
            ...
            const metrics = await socialMediaManager.getPostMetrics(account.platform, post.platformPostId);
            if (metrics) {
    ...
    // Get scheduled posts
    app.get("/api/social-media/scheduled", authenticateToken, async (req, res) => {
        try {
            if (!req.user?.id) {
                return res.status(401).json({ message: "Authentication required" });
            }
    ...
    app.delete("/api/social-media/scheduled/:scheduleId", authenticateToken, async (req, res) => {
        try {
            if (!req.user?.id) {
                return res.status(401).json({ message: "Authentication required" });
            }
