...
import { SafetyManager as SafetySystems } from './lib/safety-systems.js';
...
export async function runDeploymentTests() {
    ...
    try {
        ...
        const safetyCheck = await SafetySystems.performSafetyCheck('1', 'test_subreddit', 'Test Title', 'Test body content');
        if (safetyCheck) {
            results.passed++;
    ...
    try {
        ...
        const duplicateCheck = await SafetySystems.checkDuplicate('1', 'test_subreddit', 'Test Title', 'Test body content');
        if (duplicateCheck) {
            results.passed++;
    ...
    try {
        ...
        const platforms = ['reddit', 'twitter', 'instagram', 'tiktok', 'onlyfans'];
        const styles = ['playful', 'mysterious', 'bold', 'elegant', 'confident'];
