...
export const preGeneratedTemplates = [
    // Teasing Content
    {
        id: 'tease_001',
        category: 'teasing',
...
export function getRandomTemplates(count = 3, category, style, promotionLevel, subCategory) {
    ...
    if (promotionLevel) {
        filtered = filtered.filter(t => t.promotionLevel === promotionLevel || !t.hasOwnProperty('promotionLevel'));
    }
    if (subCategory) {
        filtered = filtered.filter(t => t.subCategory === subCategory);
    }
    // Prioritize exact matches, then partial matches
    const exactMatches = filtered.filter(t => (!category || t.category === category) &&
        (!style || t.style === style) &&
        (!promotionLevel || t.promotionLevel === promotionLevel));
...
export function addWatermark(content, isTitle = false) {
...
export function getTemplateByMood(mood) {
...
export function getTrendingTemplates(count = 5) {
    // In production, this would pull from analytics
    // For now, return templates with 'hot', 'trending', or 'viral' tags
    const trending = preGeneratedTemplates.filter(t => t.tags.some(tag => ['sale', 'discount', 'exclusive', 'limited'].includes(tag)));
