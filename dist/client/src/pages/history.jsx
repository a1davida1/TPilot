...
export default function HistoryPage() {
    ...
    const filteredGenerations = generations.filter(gen => {
        ...
        const matchesSearch = searchQuery === '' ||
            ...
            titles.some((title) => title.toLowerCase().includes(searchQuery.toLowerCase()));
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <Input placeholder="Search by title or content..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="border-gray-300"/>
              </div>
        ...
        {isLoading ? (<div className="text-center py-12">
          ...
          </div>) : filteredGenerations.length === 0 ? (<Card className="text-center py-12 shadow-lg border-0 bg-white/80 backdrop-blur-sm">
            <CardContent>
            ...
            {filteredGenerations.map((generation) => (<Card key={generation.id} className="shadow-lg border-0 bg-white/80 backdrop-blur-sm hover:shadow-xl transition-shadow">
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <div className="flex items-center space-x-2 mb-2">
                        ...
                        {generation.subreddit && (<Badge variant="outline">r/{generation.subreddit}</Badge>)}
                      </div>
                    ...
                    <div className="flex space-x-2">
                      ...
                      <Button size="sm" variant="outline" onClick={() => saveMutation.mutate(generation)} disabled={saveMutation.isPending}>
                        <ThottoPilotLogo size="sm"/>
                      </Button>
                      <Button size="sm" variant="outline" onClick={() => deleteMutation.mutate(generation.id.toString())} disabled={deleteMutation.isPending} className="text-red-600 hover:text-red-700">
                ...
                <CardContent>
                  ...
                  {Array.isArray(generation.titles) && generation.titles.length > 1 && (<div>
                      <h4 className="text-sm font-medium text-gray-900 mb-2">Alternative Titles:</h4>
                      <div className="space-y-1">
                        {generation.titles.slice(1).map((title, index) => (<div key={index} className="text-sm text-gray-600 bg-gray-100 rounded px-2 py-1">
                            {title}
                          </div>))}
                      </div>
                    </div>)}
