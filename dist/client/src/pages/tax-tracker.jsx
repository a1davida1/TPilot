...
const TaxTracker = ({ userTier = 'free' }) => {
    ...
    const handleReceiptUpload = () => {
        if (!receiptFile || !receiptExpenseId)
            return;
    ...
    const getDaysWithExpenses = () => {
        ...
        return daysInMonth.map(day => {
            const dayExpenses = calendarExpenses.filter((expense) => isSameDay(parseISO(expense.date), day));
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-purple-50">
      <div className="max-w-7xl mx-auto p-6 space-y-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-xl">
                ...
                <CardContent className="space-y-4">
                  <Button onClick={() => setShowExpenseModal(true)} className="w-full bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white shadow-lg" data-testid="button-add-expense" aria-label="Open form to add a new tax-deductible expense">
                    <Plus className="mr-2 h-4 w-4"/>
                    Add New Expense
                  </Button>
                  <Button onClick={() => setShowReceiptModal(true)} variant="outline" className="w-full border-purple-200 hover:bg-purple-50" data-testid="button-upload-receipt" aria-label="Upload receipt image or PDF for existing expense">
                    <Receipt className="mr-2 h-4 w-4"/>
                    Upload Receipt
                  </Button>
                  <Button onClick={() => setActiveTab('calendar')} variant="outline" className="w-full border-purple-200 hover:bg-purple-50" data-testid="button-view-calendar">
              ...
              <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-xl">
                ...
                <CardContent className="space-y-3">
                  {recentExpenses.length === 0 ? (<div className="text-center py-8">
                      <Receipt className="h-12 w-12 text-gray-400 mx-auto mb-3"/>
                      <p className="text-gray-500 mb-4">No expenses tracked yet</p>
                      <Button onClick={() => setShowExpenseModal(true)} size="sm" className="bg-gradient-to-r from-purple-500 to-pink-500">
                        Add Your First Expense
                      </Button>
                    </div>) : (recentExpenses.slice(0, 3).map((expense, index) => (<div key={expense.id} className="flex items-center justify-between p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                        <div>
          ...
          <TabsContent value="categories" className="space-y-6">
            ...
            return (<div key={category.id}>
                    <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-xl hover:shadow-2xl transition-all duration-300 cursor-pointer group" onClick={() => setSelectedCategory(category)}>
                      <CardContent className="p-6">
                        <div className="space-y-4">
                          <div className="flex items-center justify-between">
                            <div className="p-3 rounded-xl text-white shadow-lg" style={{ backgroundColor: category.color }}>
                          ...
                          <div className="pt-2 border-t border-gray-100">
                            ...
                            <div className="flex flex-wrap gap-1">
                              {category.examples.slice(0, 3).map((example, idx) => (<Badge key={idx} variant="outline" className="text-xs">
                                  {example}
                                </Badge>))}
                              {category.examples.length > 3 && (<Badge variant="outline" className="text-xs">
                  ...
                  <Card className="bg-white/90 backdrop-blur-sm border-0 shadow-xl">
                    <CardHeader>
                      <CardTitle className="flex items-center space-x-3">
                        <div className="p-2 rounded-lg text-white" style={{ backgroundColor: selectedCategory.color }}>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                          {selectedCategory.examples.map((example, idx) => (<div key={idx} className="flex items-center space-x-2 p-2 bg-gray-50 rounded-lg">
            ...
            <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-xl">
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    <Button variant="outline" size="sm" onClick={() => setCalendarDate(new Date(calendarDate.getFullYear(), calendarDate.getMonth() - 1))}>
                      Previous
                    </Button>
              ...
              <CardContent>
                <div className="grid grid-cols-7 gap-2 mb-4">
                  {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (<div key={day} className="text-center text-sm font-medium text-gray-500 py-2">
                      {day}
                    </div>))}
                </div>
                <div className="grid grid-cols-7 gap-2">
                  {getDaysWithExpenses().map((dayData, index) => (<div key={index} className={`
                      ...
                      {dayData.totalAmount > 0 && (<div className="space-y-1">
        ...
        ].map((item, idx) => (<div key={idx} className="flex items-center space-x-2">
                      <div className="w-2 h-2 rounded-full bg-green-500"></div>
        ...
        ].map((tip, idx) => (<div key={idx} className="flex items-center space-x-2">
          ...
          <DialogContent className="max-w-md">
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input id="description" value={expenseForm.description} onChange={(e) => setExpenseForm({ ...expenseForm, description: e.target.value })} placeholder="e.g., Professional makeup session" data-testid="input-expense-description"/>
              </div>
              
              <div>
                <Label htmlFor="amount">Amount</Label>
                <Input id="amount" type="number" step="0.01" value={expenseForm.amount} onChange={(e) => setExpenseForm({ ...expenseForm, amount: e.target.value })} placeholder="0.00" data-testid="input-expense-amount"/>
              </div>
              ...
              <div>
                ...
                <Select value={expenseForm.category} onValueChange={(value) => setExpenseForm({ ...expenseForm, category: value })}>
                  ...
                  <SelectContent>
                    {expenseCategories.map(category => (<SelectItem key={category.id} value={category.name}>
                        {category.name}
                      </SelectItem>))}
                  </SelectContent>
                </Select>
              ...
              <div>
                ...
                <Input id="date" type="date" value={expenseForm.date} onChange={(e) => setExpenseForm({ ...expenseForm, date: e.target.value })} data-testid="input-expense-date"/>
              </div>
              
              <div>
                <Label htmlFor="notes">Notes (Optional)</Label>
                <Textarea id="notes" value={expenseForm.notes} onChange={(e) => setExpenseForm({ ...expenseForm, notes: e.target.value })} placeholder="Additional details about this expense" data-testid="textarea-expense-notes"/>
              </div>
              
              <div className="flex space-x-2 pt-4">
                <Button variant="outline" onClick={() => setShowExpenseModal(false)} className="flex-1" data-testid="button-cancel-expense">
                  Cancel
                </Button>
                <Button onClick={handleCreateExpense} disabled={!expenseForm.description || !expenseForm.amount || !expenseForm.category || createExpenseMutation.isPending} className="flex-1 bg-gradient-to-r from-purple-500 to-pink-500" data-testid="button-create-expense">
          ...
          <DialogContent className="max-w-md">
            ...
            <div className="space-y-4">
              <Select value={receiptExpenseId} onValueChange={setReceiptExpenseId}>
                ...
                <SelectContent>
                  {recentExpenses.map((exp) => (<SelectItem key={exp.id} value={String(exp.id)}>
                      {exp.description}
                    </SelectItem>))}
                </SelectContent>
              </Select>
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
                {receiptFile ? (<div className="space-y-2">
                    ...
                    <Button variant="outline" size="sm" onClick={() => setReceiptFile(null)} data-testid="button-remove-receipt">
                    ...
                    <Input type="file" accept="image/*,.pdf" onChange={(e) => setReceiptFile(e.target.files?.[0] || null)} className="hidden" id="receipt-upload" data-testid="input-receipt-file"/>
                    <Label htmlFor="receipt-upload" className="cursor-pointer inline-block px-4 py-2 bg-purple-50 text-purple-600 rounded-lg hover:bg-purple-100 transition-colors">
                      Choose File
                    </Label>
                  </div>)}
              </div>
              ...
              <div className="flex space-x-2 pt-4">
                <Button variant="outline" onClick={() => setShowReceiptModal(false)} className="flex-1" data-testid="button-cancel-receipt">
                  Cancel
                </Button>
                <Button onClick={handleReceiptUpload} disabled={!receiptFile} className="flex-1 bg-gradient-to-r from-purple-500 to-pink-500" data-testid="button-upload-receipt">
                  Upload Receipt
                </Button>
              </div>
            </div>
