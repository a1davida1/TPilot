...
import { Sparkles, Mail, Lock, User, Eye, EyeOff, Shield, Target, Brain } from 'lucide-react';
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="reset-email" type="email" placeholder="your.email@example.com" value={resetEmail} onChange={(e) => setResetEmail(e.target.value)} className="pl-9" data-testid="input-reset-email"/>
              </div>
            </div>
            <Button onClick={handleForgotPassword} disabled={forgotPasswordMutation.isPending} className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700" data-testid="button-send-reset">
              {forgotPasswordMutation.isPending ? 'Sending...' : 'Send Reset Link'}
            </Button>
            <Button onClick={() => setView('login')} variant="ghost" className="w-full" data-testid="button-back-to-login">
              Back to Login
            </Button>
          </CardContent>
        </Card>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
            <div className="text-center">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
                <Button onClick={() => setView('signup')} className="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 text-white font-bold px-10 py-5 text-xl shadow-2xl border-2 border-white/20 hover:shadow-purple-500/30 hover:scale-105 transition-all duration-300" style={{
                background: 'linear-gradient(135deg, #4f46e5 0%, #7c3aed 50%, #ec4899 100%)',
                boxShadow: '0 20px 40px rgba(79, 70, 229, 0.3), 0 0 0 1px rgba(255, 255, 255, 0.2) inset'
            }}>
                  Get Started Free
                </Button>
                <Button onClick={() => setView('login')} className="bg-white/90 backdrop-blur-sm border-2 border-indigo-600 text-indigo-700 hover:bg-indigo-600 hover:text-white font-bold px-10 py-5 text-xl shadow-xl transition-all duration-300">
                  Sign In
                </Button>
              </div>
              
              <div className="text-center mb-16">
                <Button onClick={() => setLocation('/demo')} variant="ghost" className="bg-gray-800/10 backdrop-blur-sm text-gray-800 hover:bg-indigo-600 hover:text-white underline font-semibold text-lg px-6 py-3 rounded-full border border-gray-300 transition-all duration-300">
    ...
    return (<div className="min-h-screen bg-white flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-6">
          <Button variant="ghost" onClick={() => setView('landing')} className="text-foreground hover:text-purple-600 font-medium transition-colors duration-200">
            ← Back to Home
          </Button>
        </div>
        ...
        <Card className="shadow-xl border-0 bg-white/80 backdrop-blur-sm">
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              {view === 'signup' && (<div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <User className="absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground h-4 w-4"/>
                  <Input id="email" type="text" placeholder="Enter your email or username" value={email} onChange={(e) => setEmail(e.target.value)} className="pl-10" required/>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                {view === 'login' && (<div className="text-right">
                    <Button type="button" variant="link" className="p-0 h-auto text-sm text-purple-600 hover:text-purple-700" onClick={() => setView('forgot-password')} data-testid="link-forgot-password">
                      Forgot password?
                    </Button>
                  </div>)}
              </div>
              ...
              <Button type="submit" className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-semibold py-3 text-lg shadow-lg hover:shadow-xl transform hover:scale-[1.02] transition-all duration-200" disabled={authMutation.isPending}>
            ...
            <div className="mt-6 text-center space-y-2">
              <p className="text-sm text-muted-foreground">
                ...
                <button type="button" onClick={() => setView(view === 'login' ? 'signup' : 'login')} className="ml-1 text-purple-600 hover:text-purple-800 font-medium">
