...
export function GenerationHistory({ onSelectGeneration }) {
    ...
    return (<div className="space-y-4">
      ...
      <div className="space-y-3 max-h-96 overflow-y-auto">
        {history.map((generation) => (<div key={generation.id} className="border rounded-lg p-4 space-y-3 hover:bg-secondary/50 transition-colors cursor-pointer" onClick={() => onSelectGeneration?.(generation)}>
            {/* Header */}
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                {generation.style && (<Badge variant="secondary" className="text-xs">
                    {generation.style}
                  </Badge>)}
              </div>
            ...
            {/* Titles */}
            {generation.titles && generation.titles.length > 0 && (<div className="space-y-2">
                <p className="text-sm font-medium">Titles:</p>
                {generation.titles.slice(0, 2).map((title, index) => (<div key={index} className="relative group">
                    ...
                    <Button variant="ghost" size="sm" onClick={(e) => {
                      ...
                      {copiedItem === `History Title ${generation.id}-${index}` ? (<Check className="h-3 w-3"/>) : (<Copy className="h-3 w-3"/>)}
                    </Button>
                  </div>))}
                {generation.titles.length > 2 && (<p className="text-xs text-muted-foreground">
                    +{generation.titles.length - 2} more titles
                  </p>)}
              </div>)}

            {/* Content Preview */}
            {generation.content && (<div className="space-y-1">
                ...
                <div className="relative group">
                  ...
                  <Button variant="ghost" size="sm" onClick={(e) => {
                    ...
                    {copiedItem === `History Content ${generation.id}` ? (<Check className="h-3 w-3"/>) : (<Copy className="h-3 w-3"/>)}
                  </Button>
                </div>
              </div>)}

            {/* Prompt */}
            {generation.prompt && (<div className="pt-2 border-t">
