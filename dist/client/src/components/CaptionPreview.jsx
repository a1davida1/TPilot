...
export function CaptionPreview({ data }) {
    ...
    return (<Card className="rounded-2xl bg-white/60 dark:bg-gray-900/60 backdrop-blur-sm">
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        {/* ALT Text */}
        {final.alt && (<div className="space-y-1">
        ...
        {/* Hashtags */}
        {final.hashtags && final.hashtags.length > 0 && (<div className="flex flex-wrap gap-2">
            {final.hashtags.map((h, index) => (<Badge key={`${h}-${index}`} variant="secondary" className="text-xs bg-gradient-to-r from-pink-100 to-purple-100 dark:from-pink-900/30 dark:to-purple-900/30">
                {h}
              </Badge>))}
          </div>)}
        ...
        <div className="grid grid-cols-2 gap-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
          ...
          <div className="space-y-1">
            ...
            <Badge variant={final.safety_level === 'normal' ? 'default' : final.safety_level === 'spicy_safe' ? 'secondary' : 'destructive'} className="text-xs">
              {final.safety_level}
            </Badge>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          <Button onClick={handleCopyCaption} variant="outline" size="sm" className="flex items-center gap-2">
            {copiedCaption ? (<>
          ...
          <Button onClick={handleCopyJSON} variant="outline" size="sm" className="flex items-center gap-2">
            {copiedJSON ? (<>
                <Check className="h-4 w-4 text-green-500"/>
                Copied!
              </>) : (<>
