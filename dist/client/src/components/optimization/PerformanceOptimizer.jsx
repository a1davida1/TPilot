...
import { Zap, TrendingUp, Star, Sparkles, CheckCircle, Play, ArrowRight } from 'lucide-react';
export default function PerformanceOptimizer() {
    ...
    if (scoreLoading || recsLoading) {
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {[...Array(6)].map((_, i) => (<Card key={i} className="bg-gray-800 border-purple-500/20 animate-pulse">
    ...
    return (<div className="space-y-6">
        ...
        <CardContent>
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="col-span-full md:col-span-1 text-center">
              <div className="relative inline-flex items-center justify-center w-32 h-32">
                <svg className="w-32 h-32 transform -rotate-90">
                  ...
                  <circle cx="64" cy="64" r="56" fill="transparent" stroke="#8B5CF6" strokeWidth="8" strokeDasharray={`${2 * Math.PI * 56}`} strokeDashoffset={`${2 * Math.PI * 56 * (1 - performanceScore.overall / 100)}`} className="transition-all duration-500"/>
                </svg>
            ...
            <div className="col-span-full md:col-span-2 space-y-4">
              {Object.entries(performanceScore).filter(([key]) => key !== 'overall').map(([key, value]) => (<div key={key} className="space-y-2">
                  <div className="flex items-center justify-between">
      ...
      <div className="flex flex-wrap gap-2">
        {['all', 'content', 'timing', 'hashtags', 'platform', 'engagement'].map((category) => (<Button key={category} variant={activeCategory === category ? "default" : "outline"} size="sm" onClick={() => setActiveCategory(category)} className={activeCategory === category
      ...
      <div className="space-y-4">
        {filteredRecommendations.map((rec) => (<Card key={rec.id} className="bg-gray-800 border-purple-500/20">
            <CardHeader>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                {rec.actions.map((action) => (<div key={action.id} className="flex items-center justify-between p-3 bg-gray-700 rounded-lg">
                    <div className="flex items-center space-x-3">
                      {action.status === 'completed' ? (<CheckCircle className="h-4 w-4 text-green-400"/>) : action.status === 'running' ? (<div className="w-4 h-4 border-2 border-purple-400 border-t-transparent rounded-full animate-spin"/>) : (<div className="w-4 h-4 border border-gray-500 rounded"/>)}
                      <span className="text-sm text-gray-300">{action.action}</span>
                      {action.automated && (<Badge variant="outline" className="border-blue-400 text-blue-400 text-xs">
                          Auto
                        </Badge>)}
                    </div>
                    {action.status === 'pending' && (<Button size="sm" variant="ghost" className="text-purple-400 hover:text-purple-300" onClick={() => applyOptimization.mutate(rec.id)} disabled={applyOptimization.isPending}>
              ...
              <div className="flex justify-end">
                <Button onClick={() => applyOptimization.mutate(rec.id)} disabled={applyOptimization.isPending} className="bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600">
      ...
      {filteredRecommendations.length === 0 && (<Card className="bg-gray-800 border-purple-500/20">
