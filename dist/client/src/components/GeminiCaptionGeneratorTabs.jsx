...
export function GeminiCaptionGeneratorTabs() {
    ...
    const [voice, setVoice] = useState("flirty_playful");
    ...
    const handleImageUpload = (e, forRewrite = false) => {
    ...
    const PlatformVoiceSelectors = () => (<>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Select value={platform} onValueChange={setPlatform}>
            ...
            <SelectContent>
              {PLATFORMS.map(p => (<SelectItem key={p.value} value={p.value}>
                  {p.label}
                </SelectItem>))}
            </SelectContent>
          </Select>
        ...
        <div className="space-y-2">
          ...
          <Select value={voice} onValueChange={setVoice}>
            ...
            <SelectContent>
              {VOICES.map(v => (<SelectItem key={v.value} value={v.value}>
                  {v.label}
                </SelectItem>))}
            </SelectContent>
          </Select>
        ...
        <div className="space-y-2">
          ...
          <Select value={mood} onValueChange={setMood}>
            ...
            <SelectContent>
              {MOODS.map(m => (<SelectItem key={m.value} value={m.value}>
                  {m.label}
                </SelectItem>))}
            </SelectContent>
          </Select>
      ...
      <div className="flex items-center space-x-2 pt-4">
        <Checkbox id="nsfw" checked={nsfw} onCheckedChange={(checked) => setNsfw(checked)} data-testid="checkbox-nsfw"/>
    ...
    return (<div className="space-y-6">
          ...
          <Card className="relative z-10">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <Input type="url" placeholder="https://example.com/image.jpg" value={imageFile ? "" : imageUrl} onChange={(e) => setImageUrl(e.target.value)} disabled={!!imageFile} data-testid="input-image-url"/>
                  ...
                  <Label htmlFor="image-upload" className="cursor-pointer">
                    <div className="border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-lg p-6 text-center hover:border-pink-400 dark:hover:border-pink-600 transition-colors">
                      {imagePreview ? (<div className="space-y-2">
                          <img src={imagePreview} alt="Preview" className="mx-auto max-h-48 rounded-lg object-contain" data-testid="img-preview"/>
                    ...
                    <Input id="image-upload" type="file" accept="image/*" onChange={(e) => handleImageUpload(e, false)} className="hidden" data-testid="input-image-file"/>
                  </Label>
                </div>
              </div>
              ...
              <Button onClick={handleGenerateImage} disabled={isGenerating || (!imageUrl && !imageFile)} className="w-full bg-gradient-to-r from-pink-500 to-purple-500 hover:from-pink-600 hover:to-purple-600 text-white" data-testid="button-generate-image">
                {isGenerating ? (<>
          ...
          <Card className="relative z-10">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="theme" type="text" placeholder="e.g., 'Morning coffee vibes' or 'Workout motivation'" value={theme} onChange={(e) => setTheme(e.target.value)} className="w-full" data-testid="input-theme"/>
              </div>

              <div className="space-y-2">
                <Label htmlFor="context">Additional Context (Optional)</Label>
                <Textarea id="context" placeholder="Any additional details or specific requirements..." value={context} onChange={(e) => setContext(e.target.value)} rows={3} className="w-full resize-none" data-testid="textarea-context"/>
              </div>
              ...
              <Button onClick={handleGenerateText} disabled={isGenerating || !theme} className="w-full bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 text-white" data-testid="button-generate-text">
                {isGenerating ? (<>
                    <Loader2 className="mr-2 h-4 w-4 animate-spin"/>
                    Generating Content...
                  </>) : (<>
          ...
          <Card className="relative z-10">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Textarea id="existing" placeholder="Paste your existing content here..." value={existingCaption} onChange={(e) => setExistingCaption(e.target.value)} rows={4} data-testid="textarea-existing-caption"/>
              </div>

              <div className="space-y-2">
                <Label htmlFor="rewrite-image">Optional: Add Image for Context</Label>
                <Input id="rewrite-image" type="url" placeholder="https://example.com/image.jpg (optional)" value={rewriteImageUrl} onChange={(e) => setRewriteImageUrl(e.target.value)} data-testid="input-rewrite-image-url"/>
                <p className="text-sm text-muted-foreground">
              ...
              <Button onClick={handleRewrite} disabled={isGenerating || !existingCaption} className="w-full bg-gradient-to-r from-green-500 to-purple-500 hover:from-green-600 hover:to-purple-600 text-white" data-testid="button-rewrite">
                {isGenerating ? (<>
                    <Loader2 className="mr-2 h-4 w-4 animate-spin"/>
                    Rewriting Content...
                  </>) : (<>
      ...
      {captionData && (<CaptionPreview data={captionData}/>)}
    </div>);
}
