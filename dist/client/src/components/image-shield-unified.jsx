...
import { Upload, Download, Shield, Image as ImageIcon, AlertCircle, CheckCircle, X, Eye, RefreshCw, Sparkles, Lock, Trash2, Crown, Zap } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { useAuth } from '@/hooks/useAuth';
import { protectImage, downloadProtectedImage, protectionPresets } from '@/lib/image-protection';
export function ImageShieldUnified({ userTier = 'guest' }) {
    const [selectedFile, setSelectedFile] = useState(null);
    const [originalImageUrl, setOriginalImageUrl] = useState(null);
    const [isProcessing, setIsProcessing] = useState(false);
    const [protectedImageUrl, setProtectedImageUrl] = useState(null);
    const [preset, setPreset] = useState('standard');
    const [customSettings, setCustomSettings] = useState(protectionPresets.standard);
    ...
    const processImage = async () => {
        ...
        try {
            ...
            toast({
                ...
                action: (<Button size="sm" variant="outline" onClick={() => downloadImage()}>
    ...
    const downloadImage = async () => {
        if (!selectedFile || !protectedImageUrl)
            return;
        try {
    ...
    return (<div className="w-full max-w-6xl mx-auto space-y-6">
      ...
      <Card className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 border-purple-500/20">
        <CardHeader>
          <div className="flex items-start justify-between">
            ...
            <div className="ml-4">
              {!isProUser ? (<Badge variant="outline" className="border-orange-500 text-orange-600 px-3 py-1">
                  <Lock className="h-3 w-3 mr-1"/>
                  Free Tier - Watermarks Applied
                </Badge>) : (<Badge className="bg-gradient-to-r from-purple-600 to-pink-600 text-white px-3 py-1">
      ...
      <Tabs defaultValue="protect" className="w-full">
          ...
          <div className="grid md:grid-cols-2 gap-6">
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg flex items-center justify-between">
                    ...
                    {selectedFile && (<Button size="sm" variant="ghost" onClick={resetAll}>
                ...
                <CardContent>
                  <div className={`border-2 border-dashed rounded-lg p-8 text-center transition-all ${dragActive
            ? 'border-purple-500 bg-purple-50 dark:bg-purple-950/20'
            : 'border-gray-300 dark:border-gray-700 hover:border-purple-400'}`} onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop}>
                    <input type="file" accept="image/*" onChange={handleFileSelect} ref={fileInputRef} className="hidden" data-testid="file-input-protect"/>
                    ...
                    {selectedFile ? (<div className="space-y-4">
                        ...
                        <Button onClick={() => fileInputRef.current?.click()} className="bg-purple-600 hover:bg-purple-700" data-testid="button-select-image">
                          <Upload className="h-4 w-4 mr-2"/>
                          Select Image
                        </Button>
                      </div>)}
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Switch checked={useCustom} onCheckedChange={setUseCustom} data-testid="switch-custom-settings"/>
                  </div>

                  {!useCustom ? (<div className="space-y-4">
                    ...
                    </div>) : (<div className="space-y-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg">
                      <div className="space-y-2">
                        <Label className="text-sm">Blur Intensity: {customSettings.blurIntensity || 1}px</Label>
                        <Slider value={[customSettings.blurIntensity || 1]} onValueChange={([value]) => setCustomSettings(prev => ({ ...prev, blurIntensity: value }))} min={0} max={3} step={0.5} data-testid="slider-blur-intensity"/>
                      </div>
                      
                      <div className="flex items-center justify-between">
                        <Label className="text-sm">Add Noise</Label>
                        <Switch checked={customSettings.addNoise || false} onCheckedChange={(checked) => setCustomSettings(prev => ({ ...prev, addNoise: checked }))} data-testid="switch-add-noise"/>
                      </div>
                      
                      <div className="space-y-2">
                        <Label className="text-sm">Resize: {customSettings.resizePercent || 90}%</Label>
                        <Slider value={[customSettings.resizePercent || 90]} onValueChange={([value]) => setCustomSettings(prev => ({ ...prev, resizePercent: value }))} min={85} max={115} step={1} data-testid="slider-resize-percent"/>
                      </div>
                      
                      <div className="space-y-2">
                        <Label className="text-sm">Crop Edges: {customSettings.cropPercent || 0}%</Label>
                        <Slider value={[customSettings.cropPercent || 0]} onValueChange={([value]) => setCustomSettings(prev => ({ ...prev, cropPercent: value }))} min={0} max={15} step={1} data-testid="slider-crop-percent"/>
                      </div>
                    </div>)}

                  {/* Process Button */}
                  <Button onClick={processImage} disabled={!selectedFile || isProcessing} className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700" size="lg" data-testid="button-protect-image">
                    {isProcessing ? (<>
            ...
            <div className="space-y-6">
              {originalImageUrl && (<Card>
                  ...
                  <CardContent>
                    {protectedImageUrl ? (<Tabs defaultValue="protected" className="w-full">
                        <TabsList className="grid w-full grid-cols-2">
                        ...
                        <TabsContent value="original" className="mt-4">
                          <img src={originalImageUrl} alt="Original" className="w-full h-auto rounded-lg border" style={{ maxHeight: '400px', objectFit: 'contain' }} data-testid="img-original"/>
                        </TabsContent>
                        <TabsContent value="protected" className="mt-4">
                          <img src={protectedImageUrl} alt="Protected" className="w-full h-auto rounded-lg border" style={{ maxHeight: '400px', objectFit: 'contain' }} data-testid="img-protected"/>
                        </TabsContent>
                      </Tabs>) : (<img src={originalImageUrl} alt="Original" className="w-full h-auto rounded-lg border" style={{ maxHeight: '400px', objectFit: 'contain' }} data-testid="img-preview"/>)}
                  </CardContent>
                </Card>)}

              {/* Download Section */}
              {protectedImageUrl && (<Card className="bg-green-50 dark:bg-green-950/20 border-green-500/20">
                  <CardHeader>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <Button onClick={downloadImage} className="w-full bg-green-600 hover:bg-green-700" size="lg" data-testid="button-download-protected">
        ...
        <TabsContent value="gallery" className="mt-6">
          {showGallery ? (<div className="space-y-6">
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-6">
                    <input type="file" accept="image/*" multiple onChange={handleGalleryUpload} className="hidden" id="gallery-upload" ref={galleryInputRef} data-testid="file-input-gallery"/>
              ...
              <Card>
                ...
                <CardContent>
                  {galleryImages.length === 0 ? (<div className="text-center py-8 text-gray-500">
                      ...
                      {galleryImages.map((image) => (<div key={image.id} className="group relative" data-testid={`gallery-image-${image.id}`}>
                          <Dialog>
                            <DialogTrigger asChild>
                              <div className="aspect-square relative overflow-hidden rounded-lg border cursor-pointer hover:shadow-lg transition-shadow">
                                <img src={image.signedUrl} alt={image.filename} className="w-full h-full object-cover"/>
                            ...
                            <DialogContent className="max-w-2xl">
                              ...
                              <div className="space-y-4">
                                <img src={image.signedUrl} alt={image.filename} className="w-full max-h-96 object-contain rounded-lg"/>
                                
                                <div className="flex gap-2 flex-wrap">
                                  <Button size="sm" variant="destructive" onClick={() => deleteImageMutation.mutate(image.id)} disabled={deleteImageMutation.isPending} data-testid={`button-delete-${image.id}`}>
