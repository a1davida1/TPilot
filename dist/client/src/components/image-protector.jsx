...
import { Upload, Download, Shield, AlertCircle, CheckCircle, X, Eye, RefreshCw, Sparkles, Lock } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { protectImage, downloadProtectedImage, protectionPresets } from '@/lib/image-protection';
export function ImageProtector({ userTier = 'guest' }) {
      ...
      <Card className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 border-purple-500/20">
        <CardHeader>
          <div className="flex items-start justify-between">
            ...
            <div className="ml-4">
              {userTier === 'guest' || userTier === 'free' ? (<Badge variant="outline" className="border-orange-500 text-orange-600 px-3 py-1">
            ...
            <CardContent className="space-y-4">
              ...
              {!useCustom ? (<div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Slider value={[customSettings.blurIntensity || 1]} onValueChange={([value]) => setCustomSettings(prev => ({ ...prev, blurIntensity: value }))} min={0} max={3} step={0.5}/>
                  </div>
                  
                  <div className="flex items-center justify-between">
                    <Label className="text-sm">Add Noise</Label>
                    <Switch checked={customSettings.addNoise || false} onCheckedChange={(checked) => setCustomSettings(prev => ({ ...prev, addNoise: checked }))}/>
                  </div>
                  
                  <div className="space-y-2">
                    <Label className="text-sm">Resize: {customSettings.resizePercent || 90}%</Label>
                    <Slider value={[customSettings.resizePercent || 90]} onValueChange={([value]) => setCustomSettings(prev => ({ ...prev, resizePercent: value }))} min={85} max={115} step={1}/>
                  </div>
                  
                  <div className="space-y-2">
                    <Label className="text-sm">Crop Edges: {customSettings.cropPercent || 0}%</Label>
                    <Slider value={[customSettings.cropPercent || 0]} onValueChange={([value]) => setCustomSettings(prev => ({ ...prev, cropPercent: value }))} min={0} max={15} step={1}/>
                  </div>
                </div>)}

              {/* Process Button */}
              <Button onClick={processImage} disabled={!selectedFile || isProcessing} className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700" size="lg">
                {isProcessing ? (<>
        ...
        <div className="space-y-6">
              ...
              <CardContent>
                {protectedImageUrl ? (<Tabs defaultValue="protected" className="w-full">
                    ...
                    <TabsContent value="original" className="mt-4">
                      <img src={originalImageUrl} alt="Original" className="w-full h-auto rounded-lg border" style={{ maxHeight: '400px', objectFit: 'contain' }}/>
                    </TabsContent>
                    <TabsContent value="protected" className="mt-4">
                      <img src={protectedImageUrl} alt="Protected" className="w-full h-auto rounded-lg border" style={{ maxHeight: '400px', objectFit: 'contain' }}/>
                    </TabsContent>
                  </Tabs>) : (<img src={originalImageUrl} alt="Original" className="w-full h-auto rounded-lg border" style={{ maxHeight: '400px', objectFit: 'contain' }}/>)}
              </CardContent>
            </Card>)}
          ...
          {protectedImageUrl && (<Card className="bg-green-50 dark:bg-green-950/20 border-green-500/20">
              ...
              <CardContent className="space-y-4">
                ...
                <Button onClick={downloadImage} className="w-full bg-green-600 hover:bg-green-700" size="lg">
