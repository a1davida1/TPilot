...
import { Play, CheckCircle, ArrowRight, ArrowLeft, Star, Trophy, Target, Sparkles, Brain, Clock, Shield, Heart, Zap, BookOpen, Users } from 'lucide-react';
// Clean onboarding steps generator function (outside component for performance)
const getOnboardingSteps = (completedSteps) => [
    {
        id: 'welcome',
        ...
        optional: false,
        component: (<div className="text-center space-y-4">
        <div className="w-24 h-24 mx-auto flex items-center justify-center">
          <img src="/thottopilot-full-logo.png" alt="ThottoPilot - Pilot Content Creation Platform" className="h-24 w-24 object-contain"/>
        </div>
        <div>
...
export default function UserOnboarding() {
    ...
    const onboardingSteps = useMemo(() => getOnboardingSteps(completedSteps), [completedSteps]);
        ...
        },
        {
          ...
          <Card className="bg-gray-700 border-purple-500/20 p-4">
            <div className="flex items-center space-x-4">
              ...
              <Button size="sm" className="bg-gradient-to-r from-purple-500 to-pink-500" onClick={startMainTutorial}>
                Start Tutorial
              </Button>
            </div>
          </Card>
    ...
    return (<div className="space-y-6">
      ...
      <Card className="bg-gray-800 border-purple-500/20">
        <CardContent className="p-6">
          ...
          <div className="grid gap-2 md:grid-cols-5">
            {onboardingSteps.map((step, index) => (<div key={step.id} className={`p-2 rounded-lg text-center cursor-pointer transition-colors ${index === currentStep
                ? 'bg-purple-600/30 border border-purple-500'
                : step.completed
                    ? 'bg-green-600/20 border border-green-500/30'
                    : 'bg-gray-700 border border-gray-600'}`} onClick={() => setCurrentStep(index)}>
                <div className="flex items-center justify-center mb-1">
                  {step.completed ? (<CheckCircle className="h-4 w-4 text-green-400"/>) : index === currentStep ? (<div className="w-4 h-4 bg-purple-400 rounded-full"/>) : (<div className="w-4 h-4 border border-gray-400 rounded-full"/>)}
                </div>
      ...
      <Card className="bg-gray-800 border-purple-500/20">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            {onboardingSteps[currentStep].optional && (<Badge variant="outline" className="border-yellow-400 text-yellow-400">
                Optional
              </Badge>)}
          </div>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="flex justify-between">
            <Button variant="outline" onClick={handlePrevious} disabled={currentStep === 0} className="border-purple-500/20">
              <ArrowLeft className="h-4 w-4 mr-2"/>
              Previous
            </Button>
            <Button onClick={handleNext} disabled={currentStep === onboardingSteps.length - 1} className="bg-gradient-to-r from-purple-500 to-pink-500">
      ...
      <Card className="bg-gray-800 border-purple-500/20">
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {tutorials.map((tutorial) => (<Card key={tutorial.id} className="bg-gray-700 border-purple-500/20 cursor-pointer hover:border-purple-500/40 transition-colors">
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      {completedTutorials.has(tutorial.id) && (<CheckCircle className="h-4 w-4 text-green-400"/>)}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => setShowTutorial(tutorial.id)} className="text-purple-400 hover:text-purple-300">
      ...
      {/* Tutorial Modal */}
      {showTutorial && (<Dialog open={!!showTutorial} onOpenChange={() => setShowTutorial(null)}>
          <DialogContent className="bg-gray-800 border-purple-500/20 max-w-2xl">
            ...
            <div className="space-y-4">
              {tutorials.find(t => t.id === showTutorial)?.steps.map((step, index) => (<div key={index} className="flex items-start space-x-4 p-4 bg-gray-700 rounded-lg">
                  ...
                  <div className="flex-1">
                    ...
                    {step.action && (<Button size="sm" variant="outline" className="border-purple-500/20 text-xs">
                        {step.action}
                      </Button>)}
                  </div>
                </div>))}
              <div className="flex justify-end space-x-2">
                <Button variant="outline" onClick={() => setShowTutorial(null)} className="border-purple-500/20">
                  Close
                </Button>
                <Button onClick={() => {
                if (showTutorial) {
