...
export function TrendingTags() {
    ...
    const allTrendingTags = trendingData?.tags || [];
      ...
      <CardContent>
        ...
        <div className="flex flex-col lg:flex-row gap-3 mb-6">
          <div className="relative flex-1">
            ...
            <Input placeholder="Search tags or subreddits..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 bg-gray-900/50 border-white/10"/>
          </div>
        ...
        <div className="space-y-3 max-h-[600px] overflow-y-auto pr-2">
          {filteredTags.map((item) => (<div key={item.tag} className="group p-3 bg-white/5 rounded-lg hover:bg-white/10 transition-all cursor-pointer" onClick={() => copyTag(item.tag)}>
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {copiedTag === item.tag ? (<Check className="h-4 w-4 text-green-400"/>) : (<Copy className="h-4 w-4 text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity"/>)}
                </div>
              </div>
