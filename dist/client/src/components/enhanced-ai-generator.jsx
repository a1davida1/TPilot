...
import { Sparkles, Brain, RefreshCw, Settings, Copy } from "lucide-react";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { apiRequest } from "@/lib/queryClient";
import { useToast } from "@/hooks/use-toast";
export function EnhancedAIGenerator({ onContentGenerated, isGuestMode = false }) {
    ...
    return (<Card className="w-full">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          ...
          {isGuestMode && (<Badge variant="secondary" className="bg-pink-100 text-pink-800">
              Guest Mode
            </Badge>)}
        </CardTitle>
        <CardDescription>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <Textarea placeholder="Add specific details about your content... (e.g., 'Cozy morning selfie with coffee')" value={customPrompt} onChange={(e) => setCustomPrompt(e.target.value)} rows={3} className="border-pink-200 focus:ring-pink-500"/>
        </div>

        {/* Advanced Settings Toggle */}
        <div className="flex items-center space-x-2">
          <Switch id="advanced-settings" checked={useAdvancedSettings} onCheckedChange={setUseAdvancedSettings}/>
        ...
        {useAdvancedSettings && (<div className="space-y-6 p-4 bg-gray-50 rounded-lg border">
            {/* Main Categories */}
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {photoTypes.map((type) => (<Button key={type.id} variant={photoType === type.id ? "default" : "outline"} className={`text-xs p-2 h-auto ${photoType === type.id ? 'bg-pink-600 text-white' : 'hover:bg-pink-50'}`} onClick={() => setPhotoType(type.id)}>
              ...
              <div className="space-y-3">
                ...
                <Select value={textTone} onValueChange={setTextTone}>
                  ...
                  <SelectContent>
                    {textTones.map((tone) => (<SelectItem key={tone.id} value={tone.id}>
                        {tone.label}
                      </SelectItem>))}
                  </SelectContent>
                </Select>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <div className="flex items-center space-x-2">
                  <Switch id="include-promotion" checked={includePromotion} onCheckedChange={setIncludePromotion}/>
              ...
              <div className="space-y-3">
                ...
                {includeHashtags && (<div className="space-y-2">
                    <Label className="text-sm text-gray-600">Choose hashtags:</Label>
                    <div className="flex flex-wrap gap-1">
                      {defaultHashtags.map((hashtag) => (<Button key={hashtag} variant={selectedHashtags.includes(hashtag) ? "default" : "outline"} size="sm" className={`text-xs h-6 px-2 ${selectedHashtags.includes(hashtag) ? 'bg-green-600 text-white' : 'hover:bg-green-50'}`} onClick={() => toggleHashtag(hashtag)}>
                          {hashtag}
                        </Button>))}
                    </div>
                  </div>)}
            ...
            {platform === "reddit" && (<div className="space-y-2">
                <Label>Subreddit (optional)</Label>
                <input type="text" placeholder="e.g., selfie, photography" value={subreddit} onChange={(e) => setSubreddit(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md"/>
              </div>)}

            {includePromotion && (<div className="space-y-2">
        ...
        {/* Generate Button */}
        <Button onClick={handleGenerate} disabled={generateContentMutation.isPending} className="w-full bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-700 hover:to-purple-700 text-white font-medium" size="lg">
        ...
        {generatedContent && (<div className="space-y-4 p-4 bg-gradient-to-br from-pink-50 to-purple-50 rounded-lg border border-pink-200">
            <h4 className="font-semibold text-lg text-pink-800">Generated Content</h4>
            
            {/* Titles */}
            {generatedContent.titles && generatedContent.titles.length > 0 && (<div className="space-y-2">
                <Label className="text-sm font-medium text-pink-700">Titles:</Label>
                {(Array.isArray(generatedContent.titles) ? generatedContent.titles : [generatedContent.titles]).map((title, index) => (<div key={index} className="relative p-3 bg-white rounded-lg border group">
                    <p className="text-sm font-medium pr-8">{title}</p>
                    <Button variant="ghost" size="sm" onClick={() => copyToClipboard(title, 'Title')} className="absolute top-2 right-2 h-6 w-6 p-0 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            {/* Content */}
            {generatedContent.content && (<div className="space-y-2">
            ...
            {/* Photo Instructions */}
            {generatedContent.photoInstructions && (<div className="space-y-2">
                ...
                <div className="relative p-3 bg-white rounded-lg border group">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => copyToClipboard(typeof generatedContent.photoInstructions === 'string'
                    ? generatedContent.photoInstructions
                    : JSON.stringify(generatedContent.photoInstructions, null, 2), 'Photo Instructions')} className="absolute top-2 right-2 h-6 w-6 p-0 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Copy className="h-3 w-3"/>
                  </Button>
                </div>
              </div>)}
        ...
        {isGuestMode && (<div className="text-center p-4 bg-pink-50 border border-pink-200 rounded-lg">
