...
import { TrendingUp, Hash, Sparkles, Copy, Check, Search, Clock, Zap, BarChart3, RefreshCw, ArrowUp, ArrowDown } from 'lucide-react';
...
export function TrendingTagsExpanded() {
    ...
    const [trendingTags, setTrendingTags] = useState(generateTrendingData());
    useEffect(() => {
    ...
    return (<div className="space-y-6">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <Button size="sm" variant="outline" onClick={handleRefresh} disabled={isRefreshing} className="border-purple-500/30">
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <Input placeholder="Search tags..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 bg-gray-900/50 border-purple-500/20"/>
        </div>
      ...
      <Card className="bg-gray-900/50 border-purple-500/20">
        <CardContent className="p-6">
          <div className="space-y-2">
            {filteredTags.map((item) => (<div key={item.tag} className="group p-4 bg-white/5 rounded-lg hover:bg-white/10 transition-all cursor-pointer" onClick={() => copyTag(item.tag)}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <Badge variant="outline" className={`text-xs ${getHeatColor(item.heat)}`}>
                          {item.heat}
                        </Badge>
                        {item.platforms.map(platform => (<Badge key={platform} variant="secondary" className="text-xs">
                            {platform}
                          </Badge>))}
                      </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {copiedTag === item.tag ? (<Check className="h-5 w-5 text-green-400"/>) : (<Copy className="h-5 w-5 text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity"/>)}
                  </div>
                </div>
              </div>))}
          </div>
          
          {filteredTags.length === 0 && (<div className="text-center py-12 text-gray-400">
