...
import { TrendingUp, Eye, Heart, DollarSign, Clock, Brain, Sparkles } from 'lucide-react';
...
// Safe array access
function safeArrayAccess(array, index, fallback) {
...
export default function SmartAnalytics() {
                ...
                return {
                    ...
                    aiInsights: {
                        bestPostingTimes: Array.isArray(analytics.bestPostingTimes)
                            ? analytics.bestPostingTimes.map((t) => String(t?.time || '12:00')).filter(Boolean)
                            : ['09:00', '19:00', '12:00'],
    ...
    if (isLoading) {
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {[...Array(4)].map((_, i) => (<Card key={i} className="bg-gray-800 border-purple-500/20 animate-pulse">
              <CardContent className="p-6">
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          <Badge className="bg-gradient-to-r from-purple-500 to-pink-500">Phase 3</Badge>
          <Button variant="outline" size="sm" onClick={() => setTimeRange(timeRange === '7d' ? '30d' : '7d')} className="border-purple-500/20">
          ...
          <Card className="bg-gray-800 border-purple-500/20">
            ...
            <CardContent>
              <div className="space-y-4">
                {(data.topPerformingContent || []).map((content, index) => (<div key={content.id} className="flex items-center justify-between p-4 bg-gray-700 rounded-lg">
                ...
                {(!data.topPerformingContent || data.topPerformingContent.length === 0) && (<div className="text-center py-8">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card className="bg-gray-800 border-purple-500/20">
              ...
              <CardContent className="space-y-3">
                {(data.aiInsights.contentRecommendations || []).map((rec, index) => (<div key={index} className="flex items-start space-x-3 p-3 bg-gray-700 rounded-lg">
            ...
            <Card className="bg-gray-800 border-purple-500/20">
              ...
              <CardContent className="space-y-3">
                {(data.aiInsights.bestPostingTimes || []).map((time, index) => (<div key={index} className="flex items-center justify-between p-3 bg-gray-700 rounded-lg">
        ...
        <TabsContent value="revenue" className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <Card className="bg-gray-800 border-purple-500/20">
              ...
              <CardContent>
                {data.revenueByPlatform && data.revenueByPlatform.length > 0 ? (<ResponsiveContainer width="100%" height={200}>
                    <PieChart>
                      <Pie data={data.revenueByPlatform} cx="50%" cy="50%" outerRadius={80} fill="#8884d8" dataKey="revenue">
                        {data.revenueByPlatform.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]}/>))}
                      </Pie>
                      <Tooltip />
                    </PieChart>
            ...
            <Card className="bg-gray-800 border-purple-500/20">
              ...
              <CardContent className="space-y-3">
                {(data.revenueByPlatform || []).map((platform, index) => (<div key={platform.platform} className="space-y-2">
                    <div className="flex items-center justify-between">
                    ...
                    <Progress value={safeNumber(platform.percentage, 0)} className="h-2" style={{ backgroundColor: COLORS[index % COLORS.length] + '20' }}/>
                  </div>))}
                {(!data.revenueByPlatform || data.revenueByPlatform.length === 0) && (<div className="text-center py-4">
