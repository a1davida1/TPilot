...
export function PolicyPreview({ subreddit, title, body, hasLink = false, onPreviewComplete }) {
    const queryClient = useQueryClient();
    const [lastPreviewResult, setLastPreviewResult] = useState(null);
    // Get user's preview gate stats
    const { data: previewStats, isLoading: statsLoading } = useQuery({
        queryKey: ['/api/user/previewStats'],
        refetchInterval: 30000, // Refresh every 30 seconds
    });
    // Get policy feature flags
    const { data: policyFlags } = useQuery({
    ...
    const canQueuePost = () => {
        if (!previewStats || !lastPreviewResult)
            return false;
        // Must meet preview gate requirements
        if (!previewStats.canQueue)
            return false;
        // Must not be blocked
        if (lastPreviewResult.state === 'block')
            return false;
        // If blockOnWarn is enabled, warnings also block
        if (policyFlags?.blockOnWarn && lastPreviewResult.state === 'warn')
            return false;
        return true;
    };
    const progressPercentage = previewStats
        ...
        : 0;
    return (<Card className="w-full" data-testid="policy-preview-card">
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        {/* Preview Gate Status */}
        {!statsLoading && previewStats && (<div className="space-y-2" data-testid="preview-gate-status">
            ...
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div className="bg-blue-600 h-2 rounded-full transition-all" style={{ width: `${progressPercentage}%` }}></div>
            </div>
        ...
        {/* Preview Button */}
        <Button onClick={handlePreview} disabled={previewMutation.isPending || !title.trim() || !subreddit.trim()} className="w-full" variant="outline" data-testid="button-preview-content">
          {previewMutation.isPending ? 'Checking Content...' : 'Preview Content'}
        </Button>

        {/* Preview Results */}
        {lastPreviewResult && (<div className="space-y-3" data-testid="preview-results">
            {/* Status Badge */}
            <div className="flex items-center gap-2">
            ...
            {/* Warnings */}
            {lastPreviewResult.warnings.length > 0 && (<div className="space-y-2">
                {lastPreviewResult.warnings.map((warning, index) => (<Alert key={index} variant={lastPreviewResult.state === 'block' ? 'destructive' : 'default'}>
            ...
            {/* Success Message */}
            {lastPreviewResult.state === 'ok' && lastPreviewResult.warnings.length === 0 && (<Alert>
        ...
        {/* Queue Status */}
        {lastPreviewResult && previewStats && (<div className="pt-2 border-t">
            ...
            {!canQueuePost() && (<p className="text-xs text-muted-foreground mt-1">
