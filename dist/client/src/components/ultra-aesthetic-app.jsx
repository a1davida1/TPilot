...
import { Sparkles, Brain, Shield, ImageIcon, TrendingUp, Zap, ArrowRight, Check, Users, Heart, Eye, MessageCircle, DollarSign, Crown, BarChart3, History, Settings, Menu, X, LogOut, FileText, Camera, Hash, Gift } from "lucide-react";
...
export function UltraAestheticApp({ isGuestMode = true }) {
    ...
    return (<div className="min-h-screen bg-background text-foreground flex dark">
      ...
      {/* Sidebar */}
      <aside className={cn("fixed left-0 top-0 h-full bg-gray-900/95 backdrop-blur-xl border-r border-purple-500/20 transition-all duration-300 z-40", sidebarOpen ? "w-64" : "w-20")}>
        {/* Logo */}
        ...
        <nav className="p-4 space-y-2">
          {navigationItems.map((item) => (<button key={item.id} onClick={() => setActiveView(item.id)} className={cn("w-full flex items-center space-x-3 px-3 py-2.5 rounded-lg transition-all", activeView === item.id
                ? "bg-purple-600/20 text-purple-300 border border-purple-500/30"
                : "hover:bg-white/5 text-gray-400 hover:text-white")}>
              {item.icon}
              {sidebarOpen && (<>
                  <span className="flex-1 text-left">{item.label}</span>
                  {item.badge && (<Badge className="bg-purple-500/20 text-purple-300 border-purple-500/50 text-xs">
                      {item.badge}
                    </Badge>)}
                </>)}
            </button>))}
        ...
        {sidebarOpen && (<div className="absolute bottom-0 left-0 right-0 p-4 border-t border-white/10">
            {isGuestMode ? (<Card className="bg-purple-600/10 border-purple-500/30">
                <CardContent className="p-4">
                  <p className="text-sm text-purple-300 mb-3">
                    <Sparkles className="inline h-4 w-4 mr-1"/>
                    Guest Mode Active
                  </p>
                  <Button className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700" onClick={() => setShowAuthModal(true)}>
                    Unlock Full Access
                  </Button>
                </CardContent>
      ...
      <main className={cn("flex-1 transition-all duration-300", sidebarOpen ? "ml-64" : "ml-20")}>
        ...
        <header className="h-16 bg-gray-950/50 backdrop-blur-xl border-b border-white/10 flex items-center justify-between px-6">
          ...
          <div className="flex items-center space-x-6">
            ...
            {/* Auth Buttons */}
            {isGuestMode ? (<div className="flex items-center space-x-3">
                <Button variant="outline" size="sm" className="border-purple-500/30 hover:bg-purple-500/10" onClick={() => setShowAuthModal(true)}>
                  Sign In
                </Button>
                <Button size="sm" className="bg-gradient-to-r from-purple-500 to-pink-500" onClick={() => setShowAuthModal(true)}>
                  Get Started
                </Button>
        ...
        <div className="p-6">
          <div className={`transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            {activeView === "generate" && (<div className="space-y-6">
                {/* Functionality Description for Guest Users */}
                {isGuestMode && (<Card className="bg-gradient-to-r from-blue-900/20 to-cyan-900/20 border-blue-500/30 overflow-hidden">
                    <CardContent className="p-8">
                      <div className="text-center space-y-4">
                        ...
                        <div className="flex justify-center gap-4 mt-6">
                          <Button className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 px-8 py-3 text-lg" onClick={() => setShowAuthModal(true)}>
                            Start Free Trial
                            <ArrowRight className="ml-2 h-5 w-5"/>
                          </Button>
                          <Button variant="outline" className="border-blue-500/50 text-blue-400 hover:bg-blue-500/10 px-8 py-3 text-lg" onClick={() => setActiveView("protect")}>
                ...
                <div className="grid lg:grid-cols-4 gap-6">
                  ...
                  <div className="lg:col-span-3 space-y-6">
                    {/* Content Generator */}
                    <SimpleContentGenerator isGuestMode={isGuestMode} onContentGenerated={() => { }}/>
                  ...
                  <div className="space-y-6">
                    ...
                    <Card className="bg-card/80 backdrop-blur-xl border-border">
                      ...
                      <CardContent>
                        <div className="space-y-3">
                          {[1, 2, 3].map((item) => (<div key={item} className="flex items-center justify-between p-2 bg-muted/30 rounded-lg text-xs">
                    ...
                    <Card className="bg-card/80 backdrop-blur-xl border-border">
                      ...
                      <CardContent className="space-y-2">
                        <Button className="w-full justify-start text-xs h-8" variant="outline" onClick={() => setActiveView("gallery")}>
                          <Camera className="mr-2 h-3 w-3"/>
                          Upload Images
                        </Button>
                        <Button className="w-full justify-start text-xs h-8" variant="outline" onClick={() => setActiveView("trending")}>
                          <Hash className="mr-2 h-3 w-3"/>
                          Trending Tags
                        </Button>
                        <Button className="w-full justify-start text-xs h-8" variant="outline" onClick={() => setActiveView("insights")}>
                  ...
                  {/* Upgrade Prompt */}
                  {isGuestMode && (<Card className="bg-gradient-to-br from-purple-600/20 to-pink-600/20 border-purple-500/30">
                      <CardHeader>
            ...
            {activeView === "trending" && (<div className="max-w-7xl mx-auto">
                <TrendingTagsExpanded />
              </div>)}

            {activeView === "insights" && (<div className="max-w-6xl mx-auto">
                <AudienceInsights />
              </div>)}

            {activeView === "communities" && (<div className="max-w-7xl mx-auto">
                <RedditCommunities />
              </div>)}

            {activeView === "dual" && (<div className="max-w-6xl mx-auto">
                <SimpleContentGenerator isGuestMode={isGuestMode} onContentGenerated={() => { }}/>
              </div>)}

            {activeView === "perks" && (<div className="max-w-7xl mx-auto">
                <ProPerks />
              </div>)}

            {activeView === "gallery" && (<div className="max-w-6xl mx-auto">
                <ImageGallery />
              </div>)}

            {activeView === "protect" && (<div className="max-w-4xl mx-auto">
                <ImageShield isGuestMode={isGuestMode} userTier={isGuestMode ? "free" : "pro"}/>
              </div>)}

            {activeView === "samples" && (<div className="max-w-6xl mx-auto">
                <SampleUpload />
              </div>)}

            {activeView === "finetune" && (<div className="max-w-4xl mx-auto">
                <IntegratedFineTuning />
              </div>)}

            {activeView === "analytics" && (<div className="space-y-6">
            ...
            ].map((metric, index) => (<Card key={index} className="bg-gray-900/50 backdrop-blur-xl border-white/10">
