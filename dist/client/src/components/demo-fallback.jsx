...
import { Sparkles, AlertCircle, CheckCircle, ArrowRight, Star, Gift } from "lucide-react";
export function DemoFallback({ error, onRetry, onSignUp }) {
    return (<div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex items-center justify-center p-4">
      <Card className="max-w-2xl w-full shadow-premium">
        ...
        <CardContent className="space-y-6">
          {error && (<Alert className="border-orange-200 bg-orange-50">
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-4">
            <Button onClick={onSignUp || (() => window.location.href = '/login')} className="flex-1 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white h-12">
              <Gift className="mr-2 h-5 w-5"/>
              Sign Up Free - No Credit Card
              <ArrowRight className="ml-2 h-5 w-5"/>
            </Button>
            
            {onRetry && (<Button onClick={onRetry} variant="outline" className="sm:w-32 h-12">
                Try Again
              </Button>)}
          </div>
