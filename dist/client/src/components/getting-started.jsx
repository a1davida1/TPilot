...
import { CheckCircle, Circle, Zap, Shield, TrendingUp, Calculator, Sparkles, ArrowRight, Users, Crown, FileText, ChevronUp, EyeOff } from 'lucide-react';
import { ThottoPilotLogo } from '@/components/thottopilot-logo';
export function GettingStarted({ userTier = 'free', onSectionSelect, isAtBottom = false, onSetupLater }) {
    ...
    if (isMinimized) {
        return (<div className="fixed bottom-4 left-4 z-30">
        <Card className="bg-gradient-to-r from-purple-50 to-pink-50 border-purple-200 shadow-lg max-w-xs">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" onClick={() => setIsMinimized(false)} className="h-8 w-8 p-0 hover:bg-purple-100">
    ...
    return (<div className="max-w-4xl mx-auto p-6 space-y-8">
      ...
      <div className="text-center space-y-4">
        <div className="flex flex-col items-center justify-center space-y-4">
          <img src="/thottopilot-full-logo.png" alt="ThottoPilot" className="h-16 w-16 object-contain"/>
        ...
        <div className="flex items-center justify-center space-x-2">
          ...
          {userTier === 'guest' && (<Badge className="bg-green-100 text-green-700">
      ...
      <div>
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            return (<div key={step.id} className={`p-4 rounded-xl border-2 transition-all duration-300 ${isCompleted
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      <button onClick={() => toggleStep(step.id)} className="flex-shrink-0" disabled={isDisabled}>
                        {isCompleted ? (<CheckCircle className="h-6 w-6 text-green-500"/>) : (<Circle className="h-6 w-6 text-gray-300"/>)}
                      </button>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center space-x-2">
                          ...
                          {step.proOnly && (<Badge className="bg-yellow-100 text-yellow-700 text-xs">Pro Feature</Badge>)}
                        </div>
                        <p className="text-sm text-gray-600 mb-2">{step.description}</p>
                        {step.details && (<p className="text-xs text-gray-500 mb-2 italic">{step.details}</p>)}
                    ...
                    <Button onClick={() => handleStepAction(step)} disabled={isDisabled} className={isCompleted ? 'bg-green-500 hover:bg-green-600' : 'bg-purple-500 hover:bg-purple-600'}>
      ...
      <div>
        <Card>
          ...
          <CardContent>
            ...
            return (<div key={feature.title} className="p-4 rounded-xl bg-gradient-to-r from-gray-50 to-gray-100 hover:shadow-md transition-all duration-300">
                    <div className="flex items-center space-x-3">
      ...
      {(userTier === 'free' || userTier === 'guest') && (<div>
