      ...
      <Tabs defaultValue="overview" className="space-y-4">
        ...
        <TabsContent value="overview" className="space-y-4">
          ...
          <div className="grid gap-4">
            ...
            return (<Card key={feature.id} className="bg-gradient-to-r from-gray-900/50 to-gray-800/50 border-gray-700/50">
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
                    ...
                    <Switch checked={feature.enabled} onCheckedChange={feature.toggle} data-testid={`switch-${feature.id}`}/>
                  </CardHeader>
                  <CardContent className="pt-0">
                    <div className="flex items-center space-x-6 text-sm">
                      {Object.entries(feature.stats).map(([key, value]) => (<div key={key} className="text-center">
          ...
          <Card className="bg-gradient-to-r from-gray-900/50 to-gray-800/50 border-gray-700/50">
            ...
            <CardContent className="space-y-6">
              <div className="space-y-2">
                ...
                <Slider value={postFrequency} onValueChange={setPostFrequency} max={10} min={1} step={1} className="w-full" data-testid="slider-post-frequency"/>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {platforms.map((platform) => (<Badge key={platform.id} variant={selectedPlatforms.includes(platform.id) ? 'default' : 'outline'} className={`cursor-pointer ${selectedPlatforms.includes(platform.id)
                ? 'bg-purple-500 hover:bg-purple-600'
                : 'border-gray-600 hover:border-purple-500'}`} onClick={() => {
                setSelectedPlatforms(prev => prev.includes(platform.id)
          ...
          {posts.length > 0 && (<Card className="bg-gradient-to-r from-gray-900/50 to-gray-800/50 border-gray-700/50">
              <CardHeader>
                <CardTitle className="text-white">Post Status</CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
                {posts.map(post => (<div key={post.id} className="flex justify-between text-sm text-gray-400">
        ...
        <TabsContent value="platforms" className="space-y-4">
          <div className="grid gap-4">
            {platforms.map((platform) => (<Card key={platform.id} className="bg-gradient-to-r from-gray-900/50 to-gray-800/50 border-gray-700/50">
                <CardHeader className="flex flex-row items-center justify-between space-y-0">
                  ...
                  <Badge variant={platform.status === 'connected' ? 'default' :
                platform.status === 'pending' ? 'secondary' : 'outline'} className={platform.status === 'connected' ? 'bg-green-500/20 text-green-400 border-green-500/20' :
                ...
                <CardContent>
                  {platform.status === 'connected' ? (<div className="flex items-center justify-between">
                    ...
                    </div>) : (<Button className="w-full bg-purple-600 hover:bg-purple-700" data-testid={`connect-${platform.id}`}>
        ...
        <TabsContent value="rules" className="space-y-4">
          <div className="grid gap-4">
            {automationRules.map((rule) => (<Card key={rule.id} className="bg-gradient-to-r from-gray-900/50 to-gray-800/50 border-gray-700/50">
                <CardHeader className="flex flex-row items-center justify-between space-y-0">
                  <div>
                    <CardTitle className="text-white">{rule.name}</CardTitle>
                    <p className="text-sm text-gray-400">{rule.description}</p>
                  </div>
                  <Badge variant={rule.status === 'active' ? 'default' : 'secondary'} className={rule.status === 'active' ? 'bg-green-500/20 text-green-400 border-green-500/20' :
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      ...
                      <div className="flex space-x-1">
                        {rule.platforms.map((platform) => (<Badge key={platform} variant="outline" className="text-xs border-gray-600">
                            {platform}
                          </Badge>))}
                      </div>
                    </div>
