...
import { Home, Brain, Shield, ImageIcon, TrendingUp, Menu, X, Camera, Hash, Users, Plus, Upload, CheckCircle, BarChart3, History, Settings, Bell, Crown, ChevronUp, Clock, Activity, Target, DollarSign, FileText, ChevronDown, Calculator } from "lucide-react";
...
export function ModernDashboard({ isRedditConnected = false, user, userTier = 'free', isAdmin = false }) {
    ...
    const handleFileUpload = async (event) => {
        ...
        window.selectedImagePreview = previewUrl;
    ...
    return (<div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      {/* Sidebar */}
      <aside className={cn("fixed left-0 top-0 h-full w-[280px] md:w-[240px] bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 z-50 overflow-y-auto transition-transform duration-300", sidebarOpen ? "translate-x-0" : (isMobile ? "-translate-x-[280px]" : "-translate-x-[240px]"))}>
        <div className="p-6">
          {/* Logo */}
          <div className="flex items-center gap-3 mb-8">
            <div className="w-10 h-10 bg-gradient-to-br from-pink-500 to-pink-600 rounded-xl flex items-center justify-center shadow-lg">
              <img src="/logo.png" alt="ThottoPilot" className="w-8 h-8 rounded-lg"/>
            </div>
            <span className="text-xl font-bold bg-gradient-to-r from-pink-500 to-pink-600 bg-clip-text text-transparent">
              ThottoPilot
            </span>
            {isMobile && (<Button variant="ghost" size="sm" className="ml-auto" onClick={() => setSidebarOpen(false)}>
                <X className="h-4 w-4"/>
              </Button>)}
          </div>
          ...
          <nav className="space-y-1">
            {["main", "create", "manage", "insights", "account"].map((section) => (<div key={section}>
                {section !== "main" && (<div className="px-3 py-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider mt-6">
                    {section}
                  </div>)}
                {navigationItems
                ...
                }} className={cn("w-full flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-lg transition-all", activeSection === item.id
                    ? "bg-pink-50 dark:bg-pink-950/20 text-pink-600 dark:text-pink-400 shadow-sm"
                    : "text-foreground hover:bg-gray-100 dark:hover:bg-gray-800")}>
                      {item.icon}
                      <span className="flex-1 text-left">{item.label}</span>
                      {item.badge && (<Badge variant="secondary" className="text-xs px-1.5 py-0.5">
                          {item.badge}
                        </Badge>)}
                    </button>))}
              </div>))}
          ...
          {!isPremium && (<Card className="mt-8 bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 text-white border-0">
              <CardContent className="p-4 text-center">
                ...
                <Button size="sm" className="bg-white text-indigo-600 hover:bg-gray-100 w-full text-xs font-medium" onClick={() => window.location.href = '/checkout'}>
                  Upgrade Now
                </Button>
              </CardContent>
            </Card>)}
      ...
      {/* Main Content */}
      <div className={cn("transition-all duration-300", sidebarOpen && !isMobile ? "ml-[240px]" : "ml-0")}>
        {/* Header */}
        <header className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              {isRedditConnected ? (<Badge className="bg-orange-100 text-orange-700 border-orange-200">
                  <FaReddit className="h-3 w-3 mr-1"/>
                  Reddit Connected
                </Badge>) : (<Button variant="outline" size="sm" onClick={async () => {
                try {
              ...
              <Button className="bg-gradient-to-r from-pink-500 to-purple-600 hover:from-pink-600 hover:to-purple-700" onClick={() => setLocation('/caption-generator')}>
        ...
        <main className="p-6">
            ...
            {stats.map((stat, index) => (<Card key={index} className="relative overflow-hidden">
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={cn("text-sm font-semibold flex items-center gap-1", stat.changeType === "positive" ? "text-green-600" : "text-red-600")}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
            ...
            <Card className="lg:col-span-2">
              ...
              <CardContent>
                <div className={cn("border-2 border-dashed rounded-xl p-8 text-center transition-all cursor-pointer group", isDragging
            ? "border-pink-500 bg-pink-50 dark:bg-pink-950/20"
            : "border-gray-300 dark:border-gray-600 hover:border-pink-400 dark:hover:border-pink-500")} onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
            ...
            if (files && files[0]) {
                ...
                if (fileInputRef.current) {
                  ...
                  <input ref={fileInputRef} id="file-upload" type="file" accept="image/jpeg,image/jpg,image/png,image/webp,image/gif" className="hidden" onChange={handleFileUpload} onClick={(e) => {
            ...
            console.log('File input clicked, value reset');
                ...
                <div className="grid grid-cols-3 gap-3 mt-6">
                  <Button variant="outline" size="sm" className="flex flex-col gap-1 h-auto py-3" onClick={(e) => {
            e.preventDefault();
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {recentActivity.map((activity) => (<div key={activity.id} className="flex items-start gap-3">
                      <div className={cn("w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0", activity.type === "posted" && "bg-green-100 dark:bg-green-950/20 text-green-600", activity.type === "generated" && "bg-blue-100 dark:bg-blue-950/20 text-blue-600", activity.type === "protected" && "bg-purple-100 dark:bg-purple-950/20 text-purple-600", activity.type === "scheduled" && "bg-orange-100 dark:bg-orange-950/20 text-orange-600")}>
                        {activity.icon}
                      </div>
            ...
            toast({
              ...
              <CardContent>
                <div className="grid grid-cols-3 gap-3">
                  {[1, 2, 3, 4, 5, 6].map((i) => (<div key={i} className="aspect-square bg-gray-100 dark:bg-gray-800 rounded-lg relative overflow-hidden group cursor-pointer hover:opacity-80 transition-opacity">
