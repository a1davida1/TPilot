...
import { User, LogOut, Settings, History, BarChart3, Menu, X } from 'lucide-react';
...
export function Header() {
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <Link href="/dashboard">
              <div className="flex items-center gap-3 cursor-pointer h-16">
                <img src="/logo.png" alt="ThottoPilot" className="h-10 w-10 object-contain hover:scale-105 transition-transform duration-200 filter drop-shadow-md"/>
            ...
            <nav className="hidden md:flex items-center gap-2 bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm rounded-full px-4 py-2 border border-pink-200/50 dark:border-pink-800/30 shadow-lg shadow-pink-500/10">
              {visibleItems.map((item) => (<Link key={item.href} href={item.href}>
                  <span className={`px-4 py-2 rounded-full text-sm font-semibold transition-all duration-300 cursor-pointer transform hover:scale-105 ${location === item.href
                ? 'bg-gradient-to-r from-pink-500 to-rose-500 text-white shadow-lg shadow-pink-500/30'
                : 'text-foreground hover:text-pink-600 hover:bg-pink-100/80 dark:hover:bg-pink-900/30'}`} data-testid={`nav-${item.label.toLowerCase().replace(' ', '-')}`}>
                    {item.label}
                  </span>
                </Link>))}
            </nav>
    ...
    {/* Generation Counter will be added to dashboard instead */}
            ...
            <div className="hidden md:flex items-center gap-3">
              {isLoading ? (<div className="animate-spin rounded-full h-8 w-8 border-2 border-pink-500 border-t-transparent"/>) : isAuthenticated ? (<DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" size="sm" className="flex items-center gap-2 px-3 py-2 hover:bg-pink-50">
            ...
            }} className="text-gray-700 hover:text-pink-600 hover:bg-pink-50" data-testid="button-sign-in">
                    Sign In
                  </Button>
                  <Button size="sm" onClick={() => {
                setAuthModalMode('signup');
                setShowAuthModal(true);
            }} className="bg-gradient-to-r from-pink-500 to-purple-500 hover:from-pink-600 hover:to-purple-600 text-white border-0 shadow-lg" data-testid="button-get-started">
                    Get Started
                  </Button>
                </div>)}
            </div>
          ...
          {/* Mobile Menu */}
          {mobileMenuOpen && (<div className="md:hidden border-t border-pink-100 bg-white/95 backdrop-blur-md">
              <div className="px-2 pt-2 pb-3 space-y-1">
                {visibleItems.map((item) => (<Link key={item.href} href={item.href} onClick={() => setMobileMenuOpen(false)}>
                    <div className={`block px-3 py-2 rounded-lg text-base font-medium transition-colors cursor-pointer ${location === item.href
                ...
                <div className="pt-4 border-t border-pink-100">
                  {isAuthenticated ? (<div className="space-y-1">
                      ...
                      <button onClick={handleLogout} className="w-full text-left px-3 py-2 text-base font-medium text-red-600 hover:bg-red-50 rounded-lg">
                        Sign Out
                      </button>
      ...
      {/* Auth Modal */}
      <AuthModal isOpen={showAuthModal} initialMode={authModalMode} onClose={() => setShowAuthModal(false)} onSuccess={() => {
