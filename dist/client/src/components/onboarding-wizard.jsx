...
import { ChevronRight, ChevronLeft, Check, Sparkles, Link2, Palette, Rocket, Users, Shield, MessageSquare, Image } from 'lucide-react';
...
export function OnboardingWizard({ onComplete, onSkip }) {
    ...
    return (<div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-purple-50 to-pink-50 dark:from-gray-900 dark:to-gray-800">
      <Card className="w-full max-w-4xl shadow-2xl">
        <CardHeader className="pb-4">
          <div className="flex items-center justify-between mb-4">
            ...
            {onSkip && (<Button variant="ghost" size="sm" onClick={onSkip}>
                Skip Setup
              </Button>)}
          </div>
          <Progress value={progress} className="h-2"/>
          <div className="flex gap-1 mt-2">
            {steps.map((step, index) => (<div key={step.id} className={`flex-1 h-1 rounded-full transition-colors ${index <= currentStep ? 'bg-primary' : 'bg-gray-200 dark:bg-gray-700'}`}/>))}
          </div>
        </CardHeader>
        <CardContent className="pt-6">
          ...
          <div className="flex justify-between mt-8">
            ...
            <Button onClick={handleNext} disabled={!steps[currentStep].isComplete}>
...
function WelcomeStep({ preferences, setPreferences }) {
    return (<div className="space-y-6">
      ...
      <RadioGroup value={preferences.contentType} onValueChange={(value) => setPreferences({ ...preferences, contentType: value })}>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <label htmlFor="sfw" className="cursor-pointer">
            <Card className={`p-4 hover:shadow-md transition-all ${preferences.contentType === 'sfw' ? 'ring-2 ring-primary' : ''}`}>
          ...
          <label htmlFor="nsfw" className="cursor-pointer">
            <Card className={`p-4 hover:shadow-md transition-all ${preferences.contentType === 'nsfw' ? 'ring-2 ring-primary' : ''}`}>
          ...
          <label htmlFor="both" className="cursor-pointer">
            <Card className={`p-4 hover:shadow-md transition-all ${preferences.contentType === 'both' ? 'ring-2 ring-primary' : ''}`}>
...
function PlatformStep({ preferences, setPreferences }) {
    const platforms = [
    ...
    const togglePlatform = (platformId) => {
        const newPlatforms = preferences.platforms.includes(platformId)
            ? preferences.platforms.filter((p) => p !== platformId)
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {platforms.map(platform => (<Card key={platform.id} className={`p-4 cursor-pointer hover:shadow-md transition-all ${preferences.platforms.includes(platform.id) ? 'ring-2 ring-primary' : ''}`} onClick={() => togglePlatform(platform.id)}>
            <div className="flex items-center justify-between">
              ...
              <Checkbox checked={preferences.platforms.includes(platform.id)} onCheckedChange={() => togglePlatform(platform.id)}/>
            </div>
          </Card>))}
      </div>
    </div>);
...
// Step 3: AI Setup
function AISetupStep({ preferences, setPreferences }) {
    const styles = [
    ...
    return (<div className="space-y-6">
      ...
      <div>
        ...
        <RadioGroup value={preferences.aiStyle} onValueChange={(value) => setPreferences({ ...preferences, aiStyle: value })}>
          <div className="space-y-3">
            {styles.map(style => (<label key={style.id} htmlFor={style.id} className="cursor-pointer">
                <Card className={`p-4 hover:shadow-md transition-all ${preferences.aiStyle === style.id ? 'ring-2 ring-primary' : ''}`}>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {tones.map(tone => (<Button key={tone.id} variant={preferences.aiTone === tone.id ? 'default' : 'outline'} onClick={() => setPreferences({ ...preferences, aiTone: tone.id })} className="h-auto py-3">
