...
export function PhotoInstructions({ currentGeneration }) {
      ...
      <Card className="bg-white rounded-xl shadow-sm border border-gray-200 sticky top-8">
        <CardContent className="p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button variant="ghost" size="sm" onClick={() => setIsExpanded(!isExpanded)} className="text-gray-500 hover:text-gray-700">
              {isExpanded ? (<ChevronUp className="h-4 w-4"/>) : (<ChevronDown className="h-4 w-4"/>)}
            </Button>
          </div>

          {instructions ? (<div className={`space-y-4 ${!isExpanded ? 'hidden' : ''}`}>
              {/* Quick Setup */}
      ...
      <Card className="bg-white rounded-xl shadow-sm border border-gray-200">
        <CardContent className="p-6">
          ...
          <div className="space-y-3">
            <Button variant="outline" className="w-full justify-start" disabled={!currentGeneration}>
