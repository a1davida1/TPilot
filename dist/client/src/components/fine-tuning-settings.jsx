...
import { Sparkles, Save, RotateCcw, Info, Ban, Palette, Type, Camera } from "lucide-react";
export function FineTuningSettings() {
    ...
    const removeProhibitedWord = (word) => {
        updateSettings({
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <Button variant="outline" onClick={resetSettings} disabled={!hasChanges} className="border-gray-700 hover:bg-gray-800">
            <RotateCcw className="h-4 w-4 mr-2"/>
            Reset
          </Button>
          <Button onClick={() => saveMutation.mutate(settings)} disabled={!hasChanges || saveMutation.isPending} className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700">
      ...
      <Card className="bg-gray-900/50 border-gray-800/50 backdrop-blur-sm p-6">
        <div className="flex items-center justify-between">
          ...
          <Switch checked={settings.fineTuningEnabled} onCheckedChange={(checked) => updateSettings({ fineTuningEnabled: checked })} className="data-[state=checked]:bg-purple-600"/>
        </div>
      </Card>
      ...
      <Card className="bg-gray-900/50 border-gray-800/50 backdrop-blur-sm p-6">
        ...
        <div className="space-y-6">
          <div>
            ...
            <Slider value={[settings.writingStyle.tone]} onValueChange={([value]) => updateSettings({
            writingStyle: { ...settings.writingStyle, tone: value }
        })} max={100} step={1} className="[&_[role=slider]]:bg-purple-600"/>
          ...
          <div>
            ...
            <Slider value={[settings.writingStyle.formality]} onValueChange={([value]) => updateSettings({
          ...
          <div>
            ...
            <Slider value={[settings.writingStyle.explicitness]} onValueChange={([value]) => updateSettings({
      ...
      <Card className="bg-gray-900/50 border-gray-800/50 backdrop-blur-sm p-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="text" value={settings.contentPreferences.themes} onChange={(e) => updateSettings({
            contentPreferences: { ...settings.contentPreferences, themes: e.target.value }
        })} className="w-full bg-gray-800/50 border border-gray-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-purple-500" placeholder="e.g., fitness, travel, fashion, lifestyle"/>
          ...
          <div>
            ...
            <input type="text" value={settings.contentPreferences.avoid} onChange={(e) => updateSettings({
            contentPreferences: { ...settings.contentPreferences, avoid: e.target.value }
        })} className="w-full bg-gray-800/50 border border-gray-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-purple-500" placeholder="e.g., politics, religion"/>
            <p className="text-xs text-gray-500 mt-1">
      ...
      <Card className="bg-gray-900/50 border-gray-800/50 backdrop-blur-sm p-6">
        ...
        <div className="space-y-4">
          <div className="flex gap-2">
            <input type="text" value={newProhibitedWord} onChange={(e) => setNewProhibitedWord(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && addProhibitedWord()} className="flex-1 bg-gray-800/50 border border-gray-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-purple-500" placeholder="Add a word to avoid..."/>
            <Button onClick={addProhibitedWord} disabled={!newProhibitedWord.trim()} className="bg-purple-600 hover:bg-purple-700">
              Add
            </Button>
          </div>

          {settings.prohibitedWords.length > 0 ? (<div className="flex flex-wrap gap-2">
              {settings.prohibitedWords.map((word) => (<Badge key={word} variant="secondary" className="bg-red-900/30 text-red-300 cursor-pointer hover:bg-red-900/50" onClick={() => removeProhibitedWord(word)}>
                  {word}
      ...
      <Card className="bg-gray-900/50 border-gray-800/50 backdrop-blur-sm p-6">
        ...
        <div className="space-y-6">
          <div>
            ...
            <Slider value={[settings.photoStyle.lighting]} onValueChange={([value]) => updateSettings({
          ...
          <div>
            ...
            <Slider value={[settings.photoStyle.mood]} onValueChange={([value]) => updateSettings({
          ...
          <div>
            ...
            <Slider value={[settings.photoStyle.composition]} onValueChange={([value]) => updateSettings({
