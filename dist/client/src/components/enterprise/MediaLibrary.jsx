    ...
    return (<div className="p-6 space-y-6">
      ...
      {usage && usage && (<Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between text-sm">
              <span>{formatFileSize(usage.usedBytes || 0)} of {formatFileSize(usage.quotaBytes || 0)} used</span>
              <span>{usage.assetsCount || 0} files</span>
            </div>
            <Progress value={usage.usedPercentage || 0} className="h-2"/>
            {(usage.usedPercentage || 0) > 80 && (<div className="p-3 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
      ...
      <Card>
        <CardContent className="p-6">
          <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-8 text-center">
            ...
            <Button onClick={() => fileInputRef.current?.click()} disabled={uploadMutation.isPending} data-testid="button-upload-media">
              {uploadMutation.isPending ? 'Uploading...' : 'Choose Files'}
            </Button>
            <input ref={fileInputRef} type="file" accept="image/*,video/*" onChange={handleFileSelect} className="hidden"/>
            {uploadProgress > 0 && (<div className="mt-4 max-w-sm mx-auto">
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (<div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {Array.from({ length: 8 }).map((_, i) => (<div key={i} className="aspect-square bg-gray-200 dark:bg-gray-700 rounded animate-pulse"/>))}
            </div>) : assets?.length === 0 ? (<div className="text-center py-8 text-gray-500">
              ...
              {assets?.map((asset) => (<div key={asset.id} className="relative group">
                  <div className="aspect-square rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-800">
                    {asset.type.startsWith('image/') ? (<img src={asset.signedUrl} alt={asset.filename} className="w-full h-full object-cover" loading="lazy"/>) : (<div className="flex items-center justify-center h-full">
                        <Film className="h-8 w-8 text-gray-400"/>
                      </div>)}
                    
                    {/* Watermark indicator */}
                    {asset.hasWatermark && (<div className="absolute top-2 right-2">
                    ...
                    <div className="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                      <Button size="sm" variant="secondary" onClick={() => setPreviewAsset(asset)} data-testid={`button-preview-${asset.id}`}>
                        <Eye className="h-4 w-4"/>
                      </Button>
                      {asset.downloadUrl && (<Button size="sm" variant="secondary" onClick={() => window.open(asset.downloadUrl, '_blank')} data-testid={`button-download-${asset.id}`}>
                          <Download className="h-4 w-4"/>
                        </Button>)}
                      <Button size="sm" variant="destructive" onClick={() => deleteMutation.mutate(asset.id)} data-testid={`button-delete-${asset.id}`}>
      ...
      <Dialog open={!!previewAsset} onOpenChange={() => setPreviewAsset(null)}>
        <DialogContent className="max-w-4xl">
          ...
          {previewAsset && (<div className="space-y-4">
              <div className="max-h-96 overflow-hidden rounded-lg">
                {previewAsset.type.startsWith('image/') ? (<img src={previewAsset.signedUrl} alt={previewAsset.filename} className="w-full h-auto"/>) : (<video controls className="w-full">
