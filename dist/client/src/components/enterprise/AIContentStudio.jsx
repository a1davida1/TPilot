        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <Textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="Describe what you want to create... e.g., 'Cozy morning vibes with coffee'" rows={3} data-testid="textarea-prompt"/>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {platformOptions.map((platform) => (<Button key={platform.value} variant={platforms.includes(platform.value) ? "default" : "outline"} size="sm" onClick={() => {
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {styleOptions.map((style) => (<Button key={style.value} variant={styleHints.includes(style.value) ? "default" : "outline"} size="sm" onClick={() => {
            ...
            <Button onClick={handleGenerate} disabled={generateMutation.isPending} className="w-full" data-testid="button-generate-content">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              Generated Content
              {generatedContent && (<Badge variant="secondary">
          ...
          <CardContent>
            {!generatedContent ? (<div className="text-center py-12 text-gray-500">
                <Sparkles className="h-12 w-12 mx-auto mb-4 opacity-50"/>
                <p>Generate content to see results here</p>
              </div>) : (<Tabs defaultValue="0" className="space-y-4">
                <TabsList className="grid w-full" style={{ gridTemplateColumns: `repeat(${generatedContent.content.length}, 1fr)` }}>
                  {generatedContent.content.map((content, index) => (<TabsTrigger key={index} value={index.toString()}>
                      {content.platform}
                    </TabsTrigger>))}
                </TabsList>

                {generatedContent.content.map((content, index) => (<TabsContent key={index} value={index.toString()} className="space-y-4">
                    {/* Titles */}
                    <div>
                      ...
                      <div className="space-y-2">
                        {content.titles.map((title, titleIndex) => (<div key={titleIndex} className="flex items-center gap-2 group">
                            <Input value={title} readOnly className="flex-1"/>
                            <Button size="sm" variant="ghost" onClick={() => copyToClipboard(title, 'Title')} className="opacity-0 group-hover:opacity-100" data-testid={`button-copy-title-${titleIndex}`}>
                    ...
                    {content.body && (<div>
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <Button size="sm" variant="ghost" onClick={() => copyToClipboard(content.body, 'Content')} data-testid="button-copy-body">
                            <Copy className="h-4 w-4"/>
                          </Button>
                        </div>
                    ...
                    {/* Photo Instructions */}
                    {content.photoInstructions && (<div>
                        <div className="flex items-center justify-between mb-2">
                          <label className="text-sm font-medium">Photo Instructions</label>
                          <Button size="sm" variant="ghost" onClick={() => copyToClipboard(content.photoInstructions, 'Instructions')} data-testid="button-copy-instructions">
                            <Copy className="h-4 w-4"/>
                          </Button>
                        </div>
                    ...
                    {/* Hashtags */}
                    {content.hashtags && content.hashtags.length > 0 && (<div>
                        <div className="flex items-center justify-between mb-2">
                          <label className="text-sm font-medium">Hashtags</label>
                          <Button size="sm" variant="ghost" onClick={() => copyToClipboard(content.hashtags.join(' '), 'Hashtags')} data-testid="button-copy-hashtags">
                            <Copy className="h-4 w-4"/>
                          </Button>
                        </div>
                        <div className="flex flex-wrap gap-1">
                          {content.hashtags.map((tag, tagIndex) => (<Badge key={tagIndex} variant="outline" className="text-xs">
