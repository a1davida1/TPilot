          ...
          <DialogContent className="max-w-2xl">
            ...
            <form onSubmit={handleScheduleSubmit} className="space-y-4">
              <div>
                ...
                <Input value={formData.subreddit} onChange={(e) => setFormData({ ...formData, subreddit: e.target.value })} placeholder="e.g., OnlyFansPromotions" data-testid="input-subreddit"/>
              </div>
              
              <div>
                <label className="block text-sm font-medium mb-2">Title</label>
                <Input value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} placeholder="Your engaging post title" data-testid="input-title"/>
              </div>
              
              <div>
                <label className="block text-sm font-medium mb-2">Body (Optional)</label>
                <Textarea value={formData.body} onChange={(e) => setFormData({ ...formData, body: e.target.value })} placeholder="Additional post content..." rows={4} data-testid="textarea-body"/>
              </div>
              
              <div>
                <label className="block text-sm font-medium mb-2">Schedule Time</label>
                <Select value={formData.scheduledAt || 'optimal'} onValueChange={(value) => setFormData({ ...formData, scheduledAt: value === 'optimal' ? undefined : value })}>
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (<div className="space-y-4">
              {Array.from({ length: 3 }).map((_, i) => (<div key={i} className="flex items-center justify-between p-4 border rounded-lg">
                  <div className="space-y-2 flex-1">
                    <div className="h-4 bg-gray-200 dark:bg-gray-700 rounded animate-pulse"/>
                    <div className="h-3 bg-gray-200 dark:bg-gray-700 rounded animate-pulse w-2/3"/>
                  </div>
                </div>))}
            </div>) : scheduledPosts?.length === 0 ? (<div className="text-center py-8 text-gray-500">
              ...
              {scheduledPosts?.map((post) => {
                ...
                return (<div key={post.id} className="border rounded-lg p-4 space-y-3">
                    <div className="flex items-start justify-between">
                      <div className="flex-1 space-y-2">
                        ...
                        {post.bodyFinal && (<p className="text-sm text-gray-600 line-clamp-3">{post.bodyFinal}</p>)}
                      </div>
                    ...
                    {post.status === 'sent' && post.resultJson?.url && (<div className="flex items-center gap-2 text-sm">
                        <CheckCircle2 className="h-4 w-4 text-green-500"/>
                        <a href={post.resultJson.url} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">
                          View on Reddit
                        </a>
                      </div>)}
                    
                    {post.status === 'failed' && post.resultJson?.error && (<div className="flex items-center gap-2 text-sm text-red-600">
