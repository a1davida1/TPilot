...
export class QueueMonitor {
  ...
  private async collectWorkerMetrics() {
    ...
    for (const worker of workers) {
      try {
        ...
        logger.error(`Failed to collect worker metrics for ${worker.name}:`, error);
      }
    }
  }
  ...
  async pauseQueue(queueName: string): Promise<boolean> {
    try {
      ...
      logger.error(`⏸️ Queue ${queueName} paused`);
      return true;
    } catch (error) {
  ...
  async resumeQueue(queueName: string): Promise<boolean> {
    try {
      ...
      logger.error(`▶️ Queue ${queueName} resumed`);
      return true;
    } catch (error) {
  ...
  async retryFailedJobs(queueName: string): Promise<number> {
    try {
      ...
      if (queue.retryFailedJobs) {
        ...
        logger.error(`🔄 Retried ${retried} failed jobs in queue ${queueName}`);
        return retried;
      }
      return 0;
    } catch (error) {
  ...
  async clearQueue(queueName: string): Promise<boolean> {
    try {
      ...
      if (queue.clearQueue) {
        ...
        logger.error(`🧹 Cleared queue ${queueName}`);
        return true;
      }
      return false;
    } catch (error) {
