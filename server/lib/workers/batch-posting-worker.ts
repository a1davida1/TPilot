...
export class BatchPostingWorker {
  ...
  private async customizeContentForSubreddit(subreddit: string, titleTemplate: string, bodyTemplate: string) {
  ...
  private async scheduleMetricsCollection(postJobId: number, redditPostId: string) {
    try {
