...
export class EnhancedCatboxService {
  ...
  static async getUserUploadStats(userId: number): Promise<{
    ...
    try {
      // Simple query for now - can be enhanced with aggregations later
      ...
      const totalDuration = uploads.reduce((sum, u) => sum + (u.uploadDuration || 0), 0);
      
      return {
        totalUploads,
        totalSize,
        successRate: totalUploads > 0 ? (successCount / totalUploads) * 100 : 100,
        averageDuration: totalUploads > 0 ? totalDuration / totalUploads : 0
      };
    } catch (error) {
