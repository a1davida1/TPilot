...
class AnalyticsService {
  ...
  async trackLogin(userId: string, method: string) {
    ...
    try {
      ...
      logger.error(...formatLogArgs('Error updating last login:', error));
    }
  }
  ...
  async getDailyActiveUsers(): Promise<number> {
    ...
    try {
      ...
      logger.error(...formatLogArgs('Error getting DAU:', error));
      return 0;
    }
  }
  ...
  async getMonthlyActiveUsers(): Promise<number> {
    ...
    try {
      ...
      logger.error(...formatLogArgs('Error getting MAU:', error));
      return 0;
    }
  }
  ...
  private async trackEvent(userId: string, event: string, properties: Record<string, unknown>) {
    ...
    if (process.env.NODE_ENV === 'development') {
      logger.error(...formatLogArgs('Analytics Event:', event, properties));
    }
  }
