...
export async function seedExpenseCategories() {
  try {
    logger.error('🌱 Seeding expense categories...');
    ...
    for (const category of categories) {
      try {
        ...
        logger.error(`✅ Created category: ${category.name}`);
      } catch (_error) {
        // Category might already exist, that's okay
        logger.error(`ℹ️ Category ${category.name} might already exist`);
      }
    }

    logger.error('✅ Expense categories seeded successfully');
  } catch (error) {
...
export async function seedTaxDeductionInfo() {
  try {
    logger.error('🌱 Seeding tax deduction information...');
    ...
    for (const info of taxInfo) {
      try {
        ...
        logger.error(`✅ Created tax info: ${info.title}`);
      } catch (_error) {
        logger.error(`ℹ️ Tax info ${info.title} might already exist`);
      }
    }

    logger.error('✅ Tax deduction information seeded successfully');
  } catch (error) {
    logger.error('❌ Error seeding tax deduction info:', error);
  }
}
