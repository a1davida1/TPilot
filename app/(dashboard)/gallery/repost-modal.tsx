...
export function RepostModal({
  ...
  return (
    <div className={overlayStyle} role="dialog" aria-modal onClick={(e) => {
      ...
      <div className={panelStyle}>
          ...
          <div>
            ...
            {asset.lastRepostedAt ? (
              <p className="mt-1 text-xs text-emerald-600">
                Last reposted {new Date(asset.lastRepostedAt).toLocaleString()}
              </p>
            ) : (
              <p className="mt-1 text-xs text-gray-500">Not reposted yet</p>
            )}
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-end gap-3">
            ...
            <button
              type="submit"
              className={`${buttonBase} bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-60 disabled:cursor-not-allowed`}
              disabled={loading}
            >
              {loading ? 'Posting…' : 'Confirm repost'}
            </button>
          </div>
        </form>
      </div>
